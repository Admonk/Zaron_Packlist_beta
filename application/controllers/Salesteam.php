...
error_reporting(0);
                 	             ...
                 	             $this->Main_model->update_commen($data,$tablename);
        			                      ...
        			                      $datas['wedding_anniversary']=str_replace('T18:30:00.000Z','', $form_data->wedding_anniversary);
                                             ...
                                             else
                                             {
        				                      ...
        				                      if($form_data->mark_sales_member!='')
        				                      {
        				                             
        				                          
        				                             
        				                          $this->db->query("UPDATE customers SET sales_team_id='".$form_data->mark_sales_member."' WHERE sales_team_id='".$userid."'");
        				                          //$this->db->query("UPDATE orders SET user_id='".$form_data->mark_sales_member."' WHERE user_id='".$userid."'");
        				                          //$this->db->query("UPDATE orders_process SET user_id='".$form_data->mark_sales_member."' WHERE user_id='".$userid."'");
        				                          //$this->db->query("UPDATE orders_quotation SET user_id='".$form_data->mark_sales_member."' WHERE user_id='".$userid."'");
	...
	public function fetch_data_log()
	{
                      ...
                      $status=$_GET['status'];
                 	  ...
                 	  $result= $this->Main_model->where_names('log_sales_team_change','staus_base',$status);
...
$results= $this->Main_model->where_names('admin_users','define_salesteam_id',$id);
	...
	public function updateTartget(){

		$form_data = json_decode(file_get_contents("php://input"));
		$target = $form_data->target;
		$id = $form_data->id;
		$res =   $this->db->query("UPDATE admin_users SET target='".$target."' WHERE id='".$id."'");
	}
