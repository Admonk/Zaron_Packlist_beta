   ...
   public function driverview()
    {
        if (isset($this->session->userdata["logged_in"])) {
            ...
            if ($this->session->userdata["logged_in"]["access"] == "12") {
                ...
                $data["vehicle"] = $this->Main_model->where_names(
                    "vehicle",
                    "deleteid",
                    "0"
                );
...
//$this->db->query("UPDATE accountheads SET opening_balance='".abs($totalbalance)."',payment_status='".$payment_status."',op='".abs($totalbalance)."',op_status='".$payment_status."' WHERE id='346'");
                               ...
                               $this->Main_model->insert_commen($datas_log,'customer_edit_log');     
                                                                                    ...
                                                                                    date_default_timezone_set("Asia/Kolkata"); 
                                                                                     ...
                                                                                     $day_log['changes'] = 'Driver Account Changes';
  ...
  public function fetch_data_sales_group()
        ...
        public function fetch_data_get_ledger_view()
    {
                     ...
                     if($formdate=='undefined' || $formdate=='0')
                     {
                             ...
                             $result=$this->db->query("SELECT * FROM all_ledgers  WHERE  deleteid='".$deleteid."'  AND party_type=2 AND `payment_date` BETWEEN '".$formdate."' AND '".$todate."' $sql ORDER BY payment_date ASC");
...
foreach ($resultopeing_balance as  $set) 
{
 ...
 $resultopeing_balance=$this->db->query("SELECT SUM(a.credits-a.debits) as totalopeingbalance FROM all_ledgers  as a  WHERE  a.deleteid='".$deleteid."' AND a.party_type=2   $sql3   ORDER BY a.id DESC");
                     $resultopeing_balance=$resultopeing_balance->result();
foreach ($resultopeing_balance as  $set) 
{
          ...
          $balance=abs($balance);
                     ...
                     $result=$this->db->query("SELECT SUM(amount) as amount,SUM(payout) as totalpaid,SUM(credits) as totalcridit,SUM(debits) as totaldebit,SUM(balance) as totalblance FROM all_ledgers  WHERE  deleteid='".$deleteid."' AND party_type=2  $sql  ORDER BY id DESC");
                     $result=$result->result();
        ...
        public function fetch_data_get_ledger_view_export()
    {
                     ...
                     if($customer_id=='')
                     {
                           ...
                           $result=$this->db->query("SELECT * FROM all_ledgers  WHERE  deleteid='".$deleteid."' AND party_type=2 AND `payment_date` BETWEEN '".$formdate."' AND '".$todate."'  ORDER BY payment_date ASC");
                           $result=$result->result();  
        ...
        public function fetch_data_get_ledger_view_export_group()
    {
                     ...
                     if($formdate=='undefined' || $formdate=='0')
                     {
                         ...
                         $result=$this->db->query("SELECT a.*,SUM(a.debits) as debitstoatal,SUM(a.credits) as creditstotal,b.name FROM all_ledgers as a LEFT JOIN driver as b ON a.customer_id=b.id  WHERE  a.deleteid=0 AND a.party_type=2  $sql GROUP BY a.customer_id ORDER BY a.payment_date ASC");
                         $result=$result->result();  
                     ...
                     else
                     {
                          ...
                          {
                              $result=$this->db->query("SELECT a.*,SUM(a.debits) as debitstoatal,SUM(a.credits) as creditstotal,b.name FROM all_ledgers as a LEFT JOIN driver as b ON a.customer_id=b.id  WHERE  a.deleteid=0 AND a.party_type=2 AND a.paid_status='".$payment_status."'  $sql GROUP BY a.customer_id ORDER BY a.payment_date ASC");
