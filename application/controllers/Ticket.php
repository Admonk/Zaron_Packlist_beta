...
error_reporting(0);
	...
	public function ticket_view($id,$view,$sub_id)
	{
									    ...
									    if(isset($this->session->userdata['logged_in']))
							            {
                                             ...
                                             $data['status']=$status;
                                                            ...
                                                            if($sub_id>0)
                                                            {
                                                                 ...
                                                                 $this->db->query("UPDATE tickets_sub SET status='1' WHERE id='".$sub_id."'");
                                                            
                                                            }
                                                            else
                                                            {
                                                                 $this->db->query("UPDATE tickets_sub SET status='1' WHERE ticket_id='".$id."' AND user_id!='".$this->userid."'");
                                                            }
                     ...
                     $array=array();
                 	 foreach ($result as  $value) {
                 	 	...
                 	 	else
                 	 	{
                            ...
                            $message = substr($value->message, 0, 90); 
	...
	public function fetchData_trash()
	{
                   ...
                   if($this->session->userdata['logged_in']['userid']=='1')
                   {
                      	 	...
                      	 	$resultreply=$this->Main_model->where_names_two_order_by('tickets','user_id',1,'reply','1','id','ASC');
                   ...
                   {
                       	    $resultcount=0;
                       	    $result= $this->Main_model->where_names_order_by('tickets','user',$this->userid,'id','ASC');
                 	        $resultreply=$this->Main_model->where_names_two_order_by('tickets','user_id',$this->userid,'reply','1','id','ASC');
                    ...
                    foreach ($resultdelete as  $valuedelete) {
                     ...
                     $i=0;
                 	 foreach ($result as  $value)
                 	 {
                                                ...
                                                $resultcount_replay=$this->db->query("SELECT * FROM `tickets_sub`  WHERE  ticket_id IN ('".$value->id."') AND status=0 AND user_id!='".$this->userid."'  ORDER BY id  DESC");
                    ...
                    $setvalue=array_merge($array2,$array);
                    rsort($setvalue);
