                     ...
                     if($formdate=='undefined' || $formdate=='0')
                     {
                          ...
                          $formdate=date("Y-m-d", strtotime('monday this week'));
                               ...
                               if($customer_id==0)
                              {
                               ...
                               $result=$this->db->query("SELECT * FROM vendor_ledger  WHERE  deleteid=0 AND `payment_date` BETWEEN '".$formdate."' AND '".$todate."'  ORDER BY id DESC");
                 	          
                              }
                              else
                              {
                                  $result=$this->db->query("SELECT * FROM vendor_ledger  WHERE  customer_id='".$customer_id."' AND deleteid=0 AND `payment_date` BETWEEN '".$formdate."' AND '".$todate."'  ORDER BY id DESC");
		...
		public function fetch_data_get_ledger_view_total()
	{
                     ...
                     if($formdate=='undefined')
                     {
                              ...
                              if($customer_id==0)
                              {
                               ...
                               $result=$this->db->query("SELECT SUM(amount) as amount,SUM(payout) as totalpaid,SUM(credits) as totalcridit,SUM(debits) as totaldebit,SUM(balance) as totalblance FROM vendor_ledger  WHERE  deleteid=0 AND `payment_date` BETWEEN '".$formdate."' AND '".$todate."'   ORDER BY id DESC");
                 	     
                              }
                              else
                              {
                                  
                                  $result=$this->db->query("SELECT SUM(amount) as amount,SUM(payout) as totalpaid,SUM(credits) as totalcridit,SUM(debits) as totaldebit,SUM(balance) as totalblance FROM vendor_ledger  WHERE  customer_id='".$customer_id."' AND deleteid=0  AND `payment_date` BETWEEN '".$formdate."' AND '".$todate."'  ORDER BY id DESC");
