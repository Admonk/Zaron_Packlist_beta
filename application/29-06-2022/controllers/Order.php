     ...
     public function sales_order_return() 
   {
                            ...
                            $resultpss= $this->Main_model->where_names($form_data->tablenamemain,'id',$form_data->order_id);
                     	    foreach ($resultpss as  $valuepss) {
                     	        ...
                     	        $driver_id=$valuepss->driver_id;
                                        ...
                                        $order_no = $neworder_id . '/' . $this->sales_id . '/RE' . $neworder_quotation_id . '/' . date('Y');
    ...
    public function getcount_transpotcount() {
        ...
        if ($this->session->userdata['logged_in']['access'] == '12') {
           ...
           $return = $this->Main_model->where_names_five_order_by_new($tablename, 'order_base', 1, 'return_status', 1, 'user_id', $this->userid, 'assign_status', '3','loading_status', '0', 'id', 'DESC');
        ...
        elseif($this->session->userdata['logged_in']['access'] == '13') 
        {
           ...
           $return = $this->Main_model->where_names_five_order_by_new($tablename, 'order_base', 1, 'return_status', 1, 'driver_id', $this->userid, 'assign_status', '3','loading_status', '0', 'id', 'DESC');
        ...
        elseif ($this->session->userdata['logged_in']['access'] == '11') {
            ...
            $re_sudule = $this->Main_model->where_names_four_forth_where_in_order_by($tablename, 'finance_status', 3, 'assign_status', '8', 'loading_status', '0','user_id', $sales_team_id, 'id', 'DESC');
              ...
              $return = $this->Main_model->where_names_four_forth_where_in_order_by($tablename, 'return_status', 1, 'assign_status', '3', 'loading_status', '0','user_id', $sales_team_id, 'id', 'DESC');
           ...
           $ready_for_driver = $this->Main_model->where_names_four_forth_where_in_order_by($tablename, 'finance_status', 3, 'assign_status', '12','loading_status', '0', 'sales_group', $sales_team_id, 'id', 'DESC');
            ...
            $return = $this->Main_model->where_names_four_forth_where_in_order_by($tablename, 'return_status', 1, 'assign_status', '3', 'loading_status', '0','sales_group', $sales_team_id, 'id', 'DESC');
           ...
           $ready_for_delivery = $this->Main_model->where_names_four_order_by_new($tablename, 'finance_status', '3', 'assign_status', '11', 'order_base', '1','loading_status', '0', 'id', 'DESC');
            ...
            $return = $this->Main_model->where_names_four_order_by_new($tablename, 'return_status', '1', 'assign_status', '3', 'order_base', '1','loading_status', '0', 'id', 'DESC');
        ...
        $array = array('ready_for_delivery' => count($ready_for_delivery),'return' => count($return),'ready_for_driver'=>count($ready_for_driver),'pending' => count($resultpending), 'proceed' => count($resultprocessed), 'cancel' => count($resultcancel), 'request' => count($resultrequest), 'transpot' => count($transpot), 'delivered' => count($delivered), 're_sudule' => count($re_sudule), 'tripstart' => count($tripstart), 'reconciliation' => count($reconciliation));
        echo json_encode($array);
    }
    ...
    public function fetch_data_table_trasport_base() {
        ...
        foreach ($result as $value) {
            ...
            $array[] = array('no' => $i, 'id' => $value->id, 'base_id' => base64_encode($value->id), 'return_status' => $value->return_status,'order_no' => $value->order_no, 'name' => $company_name, 'email' => $email, 'order_byd' => $order_byd, 'totalamount' => round($discountfulltotal), 'commission' => round($commission), 'delivery_charge' => $value->delivery_charge, 'assign_status' => $value->assign_status, 'phone' => $phone, 'reason' => $value->reason, 'address' => $address, 'order_by' => $order_by, 'order_base' => $value->finance_status, 'create_date' => date('d-m-Y', strtotime($value->create_date)), 'create_time' => $value->create_time);
            $i++;
        }
        $myData = array('PortalActivity' => $array, 'totalCount' => $count);
        echo json_encode($myData);
            ...
            if ($collectamount != 0)
            {
                ...
                if($data_address['paid_status']==1)
                {
                   ...
                   $resbankaccount = $this->Main_model->where_names('bankaccount', 'id', 25);
                    ...
                    $data_bank1['create_date'] = date('Y-m-d');
    ...
    public function order_quotation_move_finish() {
        ...
        if ($deleteid == 0) {
            ...
            foreach ($result_order as $orders) {
                ...
                {
                    $find['order_base'] = 1;
                    $find['reason'] = 'Process To Transpot (Partial)';
                }
                else
                {
                    $find['order_base'] = 1;
            ...
            $data_address['process_by'] = 'Order Processed against order id '.$find['order_no'];
     ...
     public function order_quotation_move_finish_by_paricel() {
        ...
        $datassfirst['reason'] = 'Process To Transpot (Partial)';
    ...
    public function order_quotation_move_finish_by_deilvered() {
        ...
        $result_order_product = $this->Main_model->where_names_three_order_by('all_ledgers', 'customer_id', $customer_id, 'order_no', $order_no,'party_type', 1, 'id', 'ASC');
    ...
    public function orderassign() {
        ...
        for ($i = 0;$i < count($orderid_data);$i++) {
            ...
            if ($vehicle_id_data != '') {
                ...
                $datass['reason'] = 'Ready for PickUP / loading';
                $datass['otp'] = '1234';
