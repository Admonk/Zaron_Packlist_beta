...
error_reporting(0);
	 ...
	 public function ledger()
    {
                                        ...
                                        if(isset($this->session->userdata['logged_in']))
                                        {
                                             ...
                                             $data['employee'] = $this->Main_model->where_names('admin_users','deleteid','0');
                                             $data['bankaccount'] = $this->Main_model->where_names('bankaccount','deleteid','0');
                                             $data['active_base']='route';
                                             $data['active']='route';
                                             $data['title']    = 'Employee ledger List';
                                             $data['top_nav']  = $this->load->view('commen/top_nav', $data, TRUE);
                                             $data['side_nav'] = $this->load->view('commen/side_nav', $data, TRUE);
                                             $data['footer_copy_rights'] = $this->load->view('commen/footer_copy_rights', $data, TRUE);
                                             $this->load->view('employee/ledger.php',$data);
    	 ...
    	 public function ledger_find()
    {
                                        ...
                                        if(isset($this->session->userdata['logged_in']))
                                        {
                                             $data['customer_id']=$_GET['employee_id'];
                                             $data['employee'] = $this->Main_model->where_names('admin_users','deleteid','0');
                                             $data['bankaccount'] = $this->Main_model->where_names('bankaccount','deleteid','0');
                                             $data['active_base']='route';
                                             $data['active']='route';
                                             ...
                                             $data['side_nav'] = $this->load->view('commen/side_nav', $data, TRUE);
                                             $data['footer_copy_rights'] = $this->load->view('commen/footer_copy_rights', $data, TRUE);
                                             $this->load->view('employee/ledger_find.php',$data);
      ...
      public function ledger_view()
    {
                                        ...
                                        if(isset($this->session->userdata['logged_in']))
                                        {
                                             ...
                                             $res = $this->Main_model->where_names('admin_users','id',$_GET['vendor_id']);
                                             foreach($res as $val)
                                             {
                                                    $data['name']= $val->name;
                                             }
                                             ...
                                             $data['active_base']='route';
                                             $data['active']='route';
                                             $data['title']    = 'Employee ledger List View';
                                             $data['top_nav']  = $this->load->view('commen/top_nav', $data, TRUE);
                                             $data['side_nav'] = $this->load->view('commen/side_nav', $data, TRUE);
                                             $data['footer_copy_rights'] = $this->load->view('commen/footer_copy_rights', $data, TRUE);
                                             $this->load->view('employee/ledger_view.php',$data);
		...
		public function fetch_data_get_ledger()
	{
                     ...
                     $customer_id=$_GET['customer_id'];
                 	 ...
                 	 $result= $this->Main_model->where_names('admin_users','customer_id',$customer_id);
                 	 $i=1;
                 	  $array=array();
	...
	public function fetch_data_get_ledger_view()
	{
                     ...
                     if($formdate=='undefined' || $formdate=='0')
                     {
                               ...
                               if($customer_id==0)
                              {
                               ...
                               $result=$this->db->query("SELECT * FROM employee_ledger  WHERE  deleteid=0   ORDER BY id DESC");
                 	          
                              }
                              else
                              {
                                  $result=$this->db->query("SELECT * FROM employee_ledger  WHERE  customer_id='".$customer_id."' AND deleteid=0   ORDER BY id DESC");
                     ...
                     else
                     {
                          ...
                          {
                              $result=$this->db->query("SELECT * FROM employee_ledger  WHERE `payment_date` BETWEEN '".$formdate."' AND '".$todate."' AND customer_id='".$customer_id."' AND order_no!=''  AND deleteid=0   ORDER BY id DESC");
                 	          $resultsub=$this->db->query("SELECT * FROM employee_ledger  WHERE `payment_date` BETWEEN '".$formdate."' AND '".$todate."' AND customer_id='".$customer_id."' AND order_no=''  AND deleteid=0 ORDER BY id DESC");
                 	     
                          }
                          else
                          {
                             $result=$this->db->query("SELECT * FROM employee_ledger  WHERE `payment_date` BETWEEN '".$formdate."' AND '".$todate."' AND customer_id='".$customer_id."' AND paid_status='".$payment_status."' AND order_no!='' AND deleteid=0 ORDER BY id DESC");
                 	         $resultsub=$this->db->query("SELECT * FROM employee_ledger  WHERE `payment_date` BETWEEN '".$formdate."' AND '".$todate."' AND customer_id='".$customer_id."' AND paid_status='".$payment_status."' AND order_no='' AND deleteid=0 ORDER BY id DESC");
		...
		public function fetch_data_get_ledger_view_total()
	{
                          ...
                          if($customer_id==0)
                              {
                               ...
                               $result=$this->db->query("SELECT SUM(amount) as amount,SUM(payout) as totalpaid,SUM(credits) as totalcridit,SUM(debits) as totaldebit,SUM(balance) as totalblance FROM employee_ledger  WHERE  deleteid=0   ORDER BY id DESC");
                 	     
                              }
                              else
                              {
                                  
                                  $result=$this->db->query("SELECT SUM(amount) as amount,SUM(payout) as totalpaid,SUM(credits) as totalcridit,SUM(debits) as totaldebit,SUM(balance) as totalblance FROM employee_ledger  WHERE  customer_id='".$customer_id."' AND deleteid=0   ORDER BY id DESC");
                     ...
                     else
                     {
                          ...
                          if($payment_status=='All')
                          {
                              $result=$this->db->query("SELECT SUM(amount) as amount,SUM(payout) as totalpaid,SUM(credits) as totalcridit,SUM(debits) as totaldebit,SUM(balance) as totalblance FROM employee_ledger  WHERE `payment_date` BETWEEN '".$formdate."' AND '".$todate."' AND customer_id='".$customer_id."'   AND deleteid=0   ORDER BY id DESC");
                 	          
                          }
                          else
                          {
                             $result=$this->db->query("SELECT SUM(amount) as amount,SUM(payout) as totalpaid,SUM(credits) as totalcridit,SUM(debits) as totaldebit,SUM(balance) as totalblance FROM employee_ledger  WHERE `payment_date` BETWEEN '".$formdate."' AND '".$todate."' AND customer_id='".$customer_id."' AND paid_status='".$payment_status."'  AND deleteid=0 ORDER BY id DESC");
		...
		public function fetch_data_get_ledger_view_export()
	{
                     ...
                     $resultvent= $this->Main_model->where_names('admin_users','id',$customer_id);
                      ...
                      $fulladdress= $valuess->address1.$valuess->landmark.$valuess->pincode.','.$valuess->city.','.$valuess->state;
                         ...
                         $filename=date('d-m-Y').'_'.$name."_ledger_employee"; 
                     ...
                     foreach($res as $val)
                     {
                         ...
                         $amount=$val->amount;
                          $balancetotal+=$val->balance;
                              ...
                              $this->Main_model->update_commen_where($data,'customer_id','employee_ledger');
