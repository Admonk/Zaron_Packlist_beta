                   ...
                   <table id="datatable"  class="table table-striped table-bordered" width="100%" ng-init="fetchDatagetlegderGroup(0)" >
                      <thead>
                        <tr>
                          ...
                          <th><h5 class="font-size-14 mb-0">Notes</h5></th>
...
$(document).ready(function(){
  ...
  $('#payment_mode_payoff').on('change',function(){
      ...
      if(val=='Petty Cash')
      {
          ...
          var data='<option value="0">Select Petty Cash</option> <?php foreach($bankaccount as $val) { if($val->id==25) { ?> <option seletced value="<?php echo $val->id ?>"><?php echo $val->bank_name ?>-<?php echo $val->account_no ?></option> <?php } } ?>';
          $('#bankaccount').html(data);
          $('#res_no').show();
          $('#bankaccountshow').show();
...
$scope.fetchDatagetlegderGroupTotal = function(id,fromdate,fromto,payment_status){
    ...
    $http.get('<?php echo base_url() ?>index.php/vendor/fetch_data_get_ledger_view_total?limit=10&customer_id='+id+'&formdate='+fromdate+'&todate='+fromto+'&payment_status='+payment_status).success(function(data){
        ...
        $scope.totalblance = data.totalcridit-data.totalpayment;
