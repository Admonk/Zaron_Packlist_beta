...
class Purchase extends CI_Controller {
	...
	public function fetch_data_table() {
        ...
        $i = 1;
        $array = array();
        if ($this->session->userdata['logged_in']['access'] == '111') {
   ...
   public function purchase_order_request() 
   {
                ...
                $form_data = json_decode(file_get_contents("php://input"));
                            ...
                            $resultp= $this->Main_model->where_names($form_data->tablename_sub,'id',$form_data->order_product_id);
                     	    foreach ($resultp as  $valuep) {
                     ...
                     $purchase_product_list_process = $this->Main_model->where_names_two_order_by('purchase_product_list_process', 'sale_purchase_op_id', $form_data->order_product_id, 'sale_purchase_o_no', $form_data->order_no, 'id', 'ASC');
                     if(count($purchase_product_list_process)==0)
                     {
                                        ...
                                        $basedataproduct['sale_purchase_op_id'] = $form_data->order_product_id;
                                        ...
                                        $this->db->query("UPDATE $tablename_sub SET purchase_request=1,purchase_id='".$insertid."' WHERE id='".$form_data->order_product_id."'");
