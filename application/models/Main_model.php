...
class Main_model extends CI_Model {
 ...
 public function order_last_count_customer($tablename)
        ...
        public function order_last_count_mounth_year($tablename)
        {
                                   ...
                                   $month=date('M');
                                   $year=date('Y');
                                   $query = $this->db->query("SELECT MAX(count) as id FROM $tablename  WHERE month='".$month."' AND year='".$year."' AND deleteid=0");
                                   $res= $query->num_rows();
                                   return $query->result();
	...
	public function approvalupdate($id,$tablename,$status)
         ...
         public function cancelupdate($id,$tablename)
        {
              ...
              $update = $this->db->query("UPDATE $tablename SET order_base='-1',reason='Cancelled' WHERE id='".$id."'");
              return true;
