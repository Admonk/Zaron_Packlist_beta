...
class Order extends CI_Controller {
    ...
    public function insertandupdate() {
        ...
        if ($form_data->action == 'Save') {
            if ($form_data->profile != '') {
                ...
                } elseif ($categories == 'Screw accesories' || $categories == 'Screw' || $categories == 'Cleat' || $categories == 'Fan & Base') {
                    $data['crimp'] = 0;
                    $data['unit'] = 0;
                    $data['profile'] = 0;
                    $data['nos'] = 0;
                ...
                } elseif ($categories == 'Cleat Sagrod & L angle') {
                    $data['dim_one'] = 0;
                    if (isset($profile[1])) {
                ...
                if ($categories == 'Accesories') {
                    ...
                    if ($type == 4) {
                        ...
                        if ($categories_id == '13') {
                            $profile = round($data['profile'] / $formula, 3);
                            $data['amount'] = round($profile * $data['rate'], 3);
                            $data['nos'] = round($profile);
                            $profileset = round($data['profile'] / $formula, 3);
                        ...
                        if ($checkboxformula == 3) {
                            ...
                            $dim = round($dim / 3.281, 3);
                            $data['Meter_to_Sqr_feet'] = round($subqty / 3.281, 3);
                        }
                        if ($checkboxformula == 4) {
                            $dim = $data['dim_two'] + $data['dim_two'];
                            $dim = round($dim / 1000, 3);
                            $data['Meter_to_Sqr_feet'] = round($subqty / 1000, 3);
                        }
                        if ($checkboxformula == 5) {
                            $dim = $data['dim_two'] + $data['dim_two'];
                ...
                foreach ($result as $form_data) {
                    ...
                    if ($categories == 'Accesories') {
                        ...
                        if ($type == 4) {
                            ...
                            $datass['rate'] = $rateavl;
                            $datass['Sqr_feet_to_Meter'] = round($rateavl * $qty, 3);
                            ...
                            if ($categories_id == '13') {
                                ...
                                $profilevv = round($profile / $formula, 3);
                                $datass['amount'] = round($profilevv * $rate, 3);
                                if ($uom == 2) {
                                    $datass['nos'] = round($profilevv);
                                } else {
                                    $datass['nos'] = round($profilevv, 1);
                                }
                                $profileset = round($profile / $formula, 3);
                            ...
                            if ($uom == 3) {
                                ...
                                $toatldim = round($toatldim / 3.281, 3);
                                $datass['Meter_to_Sqr_feet'] = round($subqty / 3.281, 3);
                            }
                            if ($uom == 4) {
                                $toatldim = $dim_one + $dim_two;
                                $toatldim = round($toatldim / 1000, 3);
                                $datass['Meter_to_Sqr_feet'] = round($subqty / 1000, 3);
                            }
                            if ($uom == 5) {
                                $toatldim = $dim_one + $dim_two;
                            ...
                            if ($uom == 6) {
                                $toatldim = $dim_one + $dim_two;
                                $toatldim = round($toatldim / 39.37, 3);
                                $datass['Meter_to_Sqr_feet'] = round($subqty / 39.37, 3);
                            }
                            $sqft = round($subqty, 3);
                            $mtr = $sqft * $fact;
        ...
        if ($form_data->action == 'loadcompleted') {
            ...
            $datass_appprox['reason'] = "Driver Trip Assigned";
    ...
    public function fetch_data_delivery_data() {
        ...
        $result = $this->db->query("SELECT * FROM $tablename_sub  WHERE order_id='" . $_GET['order_id'] . "' AND  paricel_mode='" . $paricel_mode . "' AND deleteid=0 AND product_id!='' ORDER BY sort_id ASC");
        $result = $result->result();
        foreach ($result as $value) {
            $amountdata = $value->rate * $value->qty;
            $amount = $amountdata + $value->commission;
            ...
            if ($value->paricel_mode == 1) {
                ...
                foreach ($customers_adddrss as $customers_adddrss_v) {
                    $addresstopariel = $customers_adddrss_v->address1 . ' ' . $customers_adddrss_v->address2 . ' ' . $customers_adddrss_v->landmark . ' ' . $customers_adddrss_v->zone . ' ' . $customers_adddrss_v->pincode;
                }
            }
            $description = "";
            $product_name = "";
    ...
    public function fetch_data_get() {
        ...
        foreach ($result as $value) {
            ...
            $array[] = array('no' => $i, 'id' => $value->id, 'same' => $same, 'imagestatus' => $imagestatus, 'order_id' => $value->order_id, 'product_name_tab' => $product_name, 'tile_material_name' => $value->tile_material_name, 'tile_material_id' => $value->tile_material_id, 'categories' => $categories, 'type' => $type, 'description' => $description, 'product_id' => $value->product_id, 'categories_id' => $value->categories_id, 'profile_tab' => $profile, 'crimp_tab' => $crimp, 'checked' => $value->checked, 'dim_two' => $value->dim_two, 'dim_one' => $value->dim_one, 'dim_three' => $value->dim_three, 'image_length' => $value->image_length, 'gst' => $value->gst, 'gst_check' => $value->gst_check, 'extra_crimp' => $value->extra_crimp, 'back_crimp' => $value->back_crimp, 'address_id' => $value->address_id, 'proudtcion_no' => $value->proudtcion_no, 'nos_tab' => $value->nos, 'unit_tab' => $value->unit, 'return_status' => $value->return_status, 'fact_tab' => $value->fact, 'uom' => $value->uom, 'billing_options' => $value->billing_options, 'commission_tab' => $value->commission, 'paricel_mode' => $value->paricel_mode, 'cate_status' => $cate_status, 'categories_id_get' => $categories_id, 'Meter_to_Sqr_feet' => round($value->Meter_to_Sqr_feet, 3), 'Sqr_feet_to_Meter' => round($value->Sqr_feet_to_Meter, 3), 'rate_tab' => $value->rate, 'cul' => $value->cul, 'modify_qty' => $modify_qty, 'qty_tab' => $qty, 'amount_tab' => round($amount, 4));
            $i++;
        }
        echo json_encode($array);
    }
    public function fetch_data_calculation() {
        ...
        foreach ($result as $value) {
            ...
            $array[] = array('no' => $i, 'id' => $value->id, 'same' => $same, 'imagestatus' => $imagestatus, 'order_id' => $value->order_id, 'product_name_tab' => $product_name, 'categories' => $categories, 'type' => $type, 'description' => $description, 'product_id' => $value->product_id, 'tile_material_name' => $value->tile_material_name, 'tile_material_id' => $value->tile_material_id, 'categories_id' => $value->categories_id, 'profile_tab' => $profile, 'crimp_tab' => $crimp, 'checked' => $value->checked, 'dim_two' => $value->dim_two, 'dim_one' => $value->dim_one, 'dim_three' => $value->dim_three, 'image_length' => $value->image_length, 'gst' => $value->gst, 'gst_check' => $value->gst_check, 'extra_crimp' => $value->extra_crimp, 'back_crimp' => $value->back_crimp, 'proudtcion_no' => $value->proudtcion_no, 'nos_tab' => $value->nos, 'unit_tab' => $value->unit, 'return_status' => $value->return_status, 'fact_tab' => $value->fact, 'uom' => $value->uom, 'billing_options' => $value->billing_options, 'commission_tab' => $value->commission, 'cate_status' => $cate_status, 'categories_id_get' => $categories_id, 'Meter_to_Sqr_feet' => round($value->Meter_to_Sqr_feet, 3), 'Sqr_feet_to_Meter' => round($value->Sqr_feet_to_Meter, 3), 'rate_tab' => $value->rate, 'cul' => $value->cul, 'qty_tab' => $qty, 'amount_tab' => round($amount, 4));
            $i++;
        }
        echo json_encode($array);
    }
    ...
    public function fetchDataCategorybase() {
        ...
        foreach ($result as $value) {
            ...
            $array[] = array('no' => $i, 'id' => $value->id, 'categories_id' => $value->categories_id, 'type' => $value->type, 'lable' => $lable, 'lable2' => $lable2, 'lablenos' => $lablenos, 'labletype' => $labletype, 'lablefact1' => $lablefact1, 'lablefact2' => $lablefact2, 'categories_name' => $value->categories_name, 'uom' => $value->uom, 'commission_total' => round($value->commission_total, 2), 'nos_total' => round($value->nos_total, 2), 'fact_total' => round($value->fact_total, 2), 'qty_total' => round($qty, 2), 'amount_total' => round($value->amount_total + $value->commission_total, 2));
            $i++;
        }
        echo json_encode($array);
    }
    ...
    public function getcounttl_price_request() {
        ...
        if ($this->session->userdata['logged_in']['access'] == '12') {
            $resultpending = $this->Main_model->where_names_four_order_by_new($tablename, 'order_base', '0', 'user_id', $this->userid, 'deleteid', '0', 'price_requerst_status', '3', 'id', 'DESC');
            $resultprocessed = $this->Main_model->where_names_four_order_by_new($tablename, 'order_base', '1', 'user_id', $this->userid, 'deleteid', '0', 'price_requerst_status', '3', 'id', 'DESC');
            $resultcancel = $this->Main_model->where_names_four_order_by_new($tablename, 'order_base', '-1', 'user_id', $this->userid, 'deleteid', '0', 'price_requerst_status', '3', 'id', 'DESC');
            $resultrequest = $this->Main_model->where_names_four_order_by_new($tablename, 'order_base', '3', 'user_id', $this->userid, 'deleteid', '0', 'price_requerst_status', '3', 'id', 'DESC');
            $purchase_team = $this->Main_model->where_names_four_order_by_new($tablename, 'order_base', '4', 'user_id', $this->userid, 'deleteid', '0', 'price_requerst_status', '3', 'id', 'DESC');
            $md_team = $this->Main_model->where_names_four_order_by_new($tablename, 'order_base', '5', 'user_id', $this->userid, 'deleteid', '0', 'price_requerst_status', '3', 'id', 'DESC');
            $finance_team = $this->Main_model->where_names_four_order_by_new($tablename, 'order_base', '7', 'user_id', $this->userid, 'deleteid', 'price_requerst_status', '3', '0', 'id', 'DESC');
            $reassign = $this->Main_model->where_names_four_order_by_new($tablename, 'order_base', '-2', 'user_id', $this->userid, 'deleteid', '0', 'price_requerst_status', '3', 'id', 'DESC');
            $waiting = $this->Main_model->where_names_four_order_by_new($tablename, 'order_base', '-3', 'user_id', $this->userid, 'deleteid', '0', 'price_requerst_status', '3', 'id', 'DESC');
            if ($tablename == 'orders') {
    ...
    public function getcount() {
        ...
        $array = array('pending' => count($resultpending), 'proceed' => count($resultprocessed), 'cancel' => count($resultcancel), 'purchase_team' => count($purchase_team), 'request' => count($resultrequest), 'requestp' => count($purchase_team), 'md_team' => count($md_team), 'reassign' => count($reassign), 'archive' => count($archive), 'finance_team' => count($finance_team), 'vendor_po_order' => count($vendor_po_order));
        echo json_encode($array);
    }
    public function getcount_finance() {
        ...
        if ($this->session->userdata['logged_in']['access'] == '12') {
            $transpot = $this->Main_model->where_names_four_order_by_new($tablename, 'order_base', 1, 'finance_status', 3, 'user_id', $this->userid, 'deleteid', '0', 'id', 'DESC');
            $delivered = $this->Main_model->where_names_four_order_by_new($tablename, 'order_base', 1, 'finance_status', 4, 'user_id', $this->userid, 'deleteid', '0', 'id', 'DESC');
            $reconciliation = $this->Main_model->where_names_four_order_by_new($tablename, 'order_base', 1, 'finance_status', 5, 'user_id', $this->userid, 'deleteid', '0', 'id', 'DESC');
        } elseif ($this->session->userdata['logged_in']['access'] == '11') {
            $sales_team_id = array();
            $query = $this->db->query("SELECT id FROM `admin_users`  WHERE define_saleshd_id='" . $this->userid . "'");
            $resultsales_team = $query->result();
        ...
        $array = array('pending' => count($resultpending), 'proceed' => count($resultprocessed), 'cancel' => count($resultcancel), 'request' => count($resultrequest), 'transpot' => count($transpot), 'delivered' => count($delivered), 'reconciliation' => count($reconciliation));
        echo json_encode($array);
    }
    public function getcount_transpotcount() {
        ...
        if ($this->session->userdata['logged_in']['access'] == '12') {
            ...
            $ready_for_delivery = $this->Main_model->where_names_five_order_by_new($tablename, 'order_base', 1, 'finance_status', 3, 'user_id', $this->userid, 'assign_status', '11','loading_status', '0', 'id', 'DESC');
          
            $ready_for_driver = $this->Main_model->where_names_five_order_by_new($tablename, 'order_base', 1, 'finance_status', 3, 'user_id', $this->userid, 'assign_status', '12','loading_status', '0', 'id', 'DESC');
          
          
            $transpot = $this->Main_model->where_names_five_order_by_new($tablename, 'order_base', 1, 'finance_status', 3, 'user_id', $this->userid, 'assign_status', '1','loading_status', '0', 'id', 'DESC');
            $delivered = $this->Main_model->where_names_five_order_by_new($tablename, 'order_base', 1, 'finance_status', 4, 'user_id', $this->userid, 'assign_status', '3','loading_status', '0', 'id', 'DESC');
            $reconciliation = $this->Main_model->where_names_five_order_by_new($tablename, 'order_base', 1, 'finance_status', 5, 'user_id', $this->userid, 'assign_status','loading_status', '0', '3', 'id', 'DESC');
            $re_sudule = $this->Main_model->where_names_five_order_by_new($tablename, 'order_base', 1, 'finance_status', 3, 'user_id', $this->userid, 'assign_status', '8','loading_status', '0', 'id', 'DESC');
            $tripstart = $this->Main_model->where_names_five_order_by_new($tablename, 'order_base', 1, 'finance_status', 3, 'user_id', $this->userid, 'assign_status', '2','loading_status', '0', 'id', 'DESC');
        ...
        elseif($this->session->userdata['logged_in']['access'] == '13') 
        {
            ...
            $ready_for_delivery = $this->Main_model->where_names_five_order_by_new($tablename, 'order_base', 1, 'finance_status', 3, 'driver_id', $this->userid, 'assign_status', '11','loading_status', '0', 'id', 'DESC');
            
            $ready_for_driver = $this->Main_model->where_names_five_order_by_new($tablename, 'order_base', 1, 'finance_status', 3, 'driver_id', $this->userid, 'assign_status', '12','loading_status', '0', 'id', 'DESC');
            
            
            $transpot = $this->Main_model->where_names_five_order_by_new($tablename, 'order_base', 1, 'finance_status', 3, 'driver_id', $this->userid, 'assign_status', '1','loading_status', '0', 'id', 'DESC');
            $delivered = $this->Main_model->where_names_five_order_by_new($tablename, 'order_base', 1, 'finance_status', 4, 'driver_id', $this->userid, 'assign_status', '3','loading_status', '0', 'id', 'DESC');
            $reconciliation = $this->Main_model->where_names_five_order_by_new($tablename, 'order_base', 1, 'finance_status', 5, 'driver_id', $this->userid, 'assign_status','loading_status', '0', '3', 'id', 'DESC');
            $re_sudule = $this->Main_model->where_names_five_order_by_new($tablename, 'order_base', 1, 'finance_status', 3, 'driver_id', $this->userid, 'assign_status', '8','loading_status', '0', 'id', 'DESC');
            $tripstart = $this->Main_model->where_names_five_order_by_new($tablename, 'order_base', 1, 'finance_status', 3, 'driver_id', $this->userid, 'assign_status', '2','loading_status', '0', 'id', 'DESC');
        ...
        elseif ($this->session->userdata['logged_in']['access'] == '11') {
            ...
            $ready_for_delivery = $this->Main_model->where_names_four_forth_where_in_order_by($tablename, 'finance_status', 3, 'assign_status', '11','loading_status', '0', 'user_id', $sales_team_id, 'id', 'DESC');
            
             $ready_for_driver = $this->Main_model->where_names_four_forth_where_in_order_by($tablename, 'finance_status', 3, 'assign_status', '12','loading_status', '0', 'user_id', $sales_team_id, 'id', 'DESC');
            
            
            $transpot = $this->Main_model->where_names_four_forth_where_in_order_by($tablename, 'finance_status', 3, 'assign_status', '1','loading_status', '0', 'user_id', $sales_team_id, 'id', 'DESC');
            $delivered = $this->Main_model->where_names_four_forth_where_in_order_by($tablename, 'finance_status', 4, 'assign_status', '3','loading_status', '0', 'user_id', $sales_team_id, 'id', 'DESC');
            $reconciliation = $this->Main_model->where_names_four_forth_where_in_order_by($tablename, 'finance_status', 5, 'assign_status', '3','loading_status', '0', 'user_id', $sales_team_id, 'id', 'DESC');
            $re_sudule = $this->Main_model->where_names_four_forth_where_in_order_by($tablename, 'finance_status', 3, 'assign_status', '8', 'loading_status', '0','user_id', $sales_team_id, 'id', 'DESC');
            $tripstart = $this->Main_model->where_names_four_forth_where_in_order_by($tablename, 'finance_status', 3, 'assign_status', '2', 'loading_status', '0','user_id', $sales_team_id, 'id', 'DESC');
            ...
            $ready_for_delivery = $this->Main_model->where_names_four_forth_where_in_order_by($tablename, 'finance_status', 3, 'assign_status', '11','loading_status', '0', 'sales_group', $sales_team_id, 'id', 'DESC');
           $ready_for_driver = $this->Main_model->where_names_four_forth_where_in_order_by($tablename, 'finance_status', 3, 'assign_status', '12','loading_status', '0', 'sales_group', $sales_team_id, 'id', 'DESC');
           
            $transpot = $this->Main_model->where_names_four_forth_where_in_order_by($tablename, 'finance_status', 3, 'assign_status', '1','loading_status', '0', 'sales_group', $sales_team_id, 'id', 'DESC');
            $delivered = $this->Main_model->where_names_four_forth_where_in_order_by($tablename, 'finance_status', 4, 'assign_status', '3','loading_status', '0', 'sales_group', $sales_team_id, 'id', 'DESC');
            $reconciliation = $this->Main_model->where_names_four_forth_where_in_order_by($tablename, 'finance_status', 5, 'assign_status', '3','loading_status', '0', 'sales_group', $sales_team_id, 'id', 'DESC');
            $re_sudule = $this->Main_model->where_names_four_forth_where_in_order_by($tablename, 'finance_status', 3, 'assign_status', '8', 'loading_status', '0','sales_group', $sales_team_id, 'id', 'DESC');
            $tripstart = $this->Main_model->where_names_four_forth_where_in_order_by($tablename, 'finance_status', 3, 'assign_status', '2', 'loading_status', '0','sales_group', $sales_team_id, 'id', 'DESC');
        } else {
           
           $ready_for_delivery = $this->Main_model->where_names_four_order_by_new($tablename, 'finance_status', '3', 'assign_status', '11', 'order_base', '1','loading_status', '0', 'id', 'DESC');
             $ready_for_driver = $this->Main_model->where_names_four_order_by_new($tablename, 'finance_status', '3', 'assign_status', '12', 'order_base', '1','loading_status', '0', 'id', 'DESC');
            
            $transpot = $this->Main_model->where_names_four_order_by_new($tablename, 'finance_status', '3', 'assign_status', '1', 'order_base', '1','loading_status', '0', 'id', 'DESC');
            $delivered = $this->Main_model->where_names_four_order_by_new($tablename, 'finance_status', '4', 'assign_status', '3', 'order_base', '1','loading_status', '0', 'id', 'DESC');
            $reconciliation = $this->Main_model->where_names_four_order_by_new($tablename, 'finance_status', '5', 'assign_status', '3', 'order_base', '1','loading_status', '0', 'id', 'DESC');
            $re_sudule = $this->Main_model->where_names_four_order_by_new($tablename, 'finance_status', '3', 'assign_status', '8', 'order_base', '1','loading_status', '0', 'id', 'DESC');
            $tripstart = $this->Main_model->where_names_four_order_by_new($tablename, 'finance_status', '3', 'assign_status', '2', 'order_base', '1','loading_status', '0', 'id', 'DESC');
        }
        $array = array('ready_for_delivery' => count($ready_for_delivery),'ready_for_driver'=>count($ready_for_driver),'pending' => count($resultpending), 'proceed' => count($resultprocessed), 'cancel' => count($resultcancel), 'request' => count($resultrequest), 'transpot' => count($transpot), 'delivered' => count($delivered), 're_sudule' => count($re_sudule), 'tripstart' => count($tripstart), 'reconciliation' => count($reconciliation));
        echo json_encode($array);
    }
    ...
    public function fetch_data_table_tl_price_request() {
        ...
        if ($this->session->userdata['logged_in']['access'] == '12') {
            $price_requerst_status = 3;
            $queryss = $this->db->query("SELECT count(id) as totalcount  FROM $tablename  WHERE deleteid='0' AND user_id='" . $this->userid . "' AND price_requerst_status='" . $price_requerst_status . "' AND order_base='" . $order_base . "'  ORDER BY id DESC");
            $resultcount = $queryss->result();
            foreach ($resultcount as $cc) {
                $count = $cc->totalcount;
            }
            $query = $this->db->query("SELECT * FROM $tablename  WHERE deleteid='0'  AND user_id='" . $this->userid . "' AND price_requerst_status='" . $price_requerst_status . "'  AND order_base='" . $order_base . "' $where ORDER BY id DESC LIMIT $offset, $pagesize");
            $result = $query->result();
        } elseif ($this->session->userdata['logged_in']['access'] == '11') {
            $sales_team_id = array();
            $query = $this->db->query("SELECT id FROM `admin_users`  WHERE define_saleshd_id='" . $this->userid . "'");
            ...
            $sales_team_id = "'" . implode("','", $sales_team_id) . "'";
            $userslog = ' AND  user_id IN (' . $sales_team_id . ')';
            $queryss = $this->db->query("SELECT count(id) as totalcount  FROM $tablename  WHERE deleteid='0'  AND price_requerst_status='" . $price_requerst_status . "' AND order_base='" . $order_base . "'  $userslog ORDER BY id DESC");
            $resultcount = $queryss->result();
            foreach ($resultcount as $cc) {
                $count = $cc->totalcount;
            }
            ...
            $price_requerst_status = 3;
            $queryss = $this->db->query("SELECT count(id) as totalcount  FROM $tablename  WHERE deleteid='0' AND price_requerst_status='" . $price_requerst_status . "' AND order_base='" . $order_base . "'  ORDER BY id DESC");
            $resultcount = $queryss->result();
            foreach ($resultcount as $cc) {
                $count = $cc->totalcount;
            }
    ...
    public function fetch_data_table_finance() {
        ...
        $array = array();
        if ($this->session->userdata['logged_in']['access'] == '12') {
            $queryss = $this->db->query("SELECT count(id) as totalcount  FROM $tablename  WHERE deleteid='0' AND order_base='1' AND finance_status='" . $order_base . "' AND user_id='" . $sales_team_id . "'  ORDER BY id DESC");
            $resultcount = $queryss->result();
            foreach ($resultcount as $cc) {
                $count = $cc->totalcount;
            }
            $query = $this->db->query("SELECT * FROM $tablename  WHERE deleteid='0' AND order_base='1' AND finance_status='" . $order_base . "' AND user_id='" . $sales_team_id . "' $where ORDER BY id DESC LIMIT $offset, $pagesize");
            $result = $query->result();
        } elseif ($this->session->userdata['logged_in']['access'] == '11') {
            $sales_team_id = array();
            $query = $this->db->query("SELECT id FROM `admin_users`  WHERE define_saleshd_id='" . $this->userid . "'");
            ...
            $sales_team_id = "'" . implode("','", $sales_team_id) . "'";
            $userslog = ' AND  user_id IN (' . $sales_team_id . ')';
            $queryss = $this->db->query("SELECT count(id) as totalcount  FROM $tablename  WHERE deleteid='0' AND order_base='1' AND finance_status='" . $order_base . "' $userslog  ORDER BY id DESC");
            $resultcount = $queryss->result();
            foreach ($resultcount as $cc) {
                $count = $cc->totalcount;
            }
            $query = $this->db->query("SELECT * FROM $tablename  WHERE deleteid='0' AND order_base='1' AND finance_status='" . $order_base . "'  $where $userslog ORDER BY id DESC LIMIT $offset, $pagesize");
            $result = $query->result();
        } elseif ($this->session->userdata['logged_in']['access'] == '16') {
            $sales_team_id = array();
            $query = $this->db->query("SELECT id FROM `sales_group`  WHERE sales_group_head='" . $this->userid . "'");
            ...
            if ($order_base == 111) {
                $queryss = $this->db->query("SELECT count(id) as totalcount  FROM $tablename  WHERE deleteid='0' AND order_base='1'  ORDER BY id DESC");
                $resultcount = $queryss->result();
                foreach ($resultcount as $cc) {
                    $count = $cc->totalcount;
                }
                $query = $this->db->query("SELECT * FROM $tablename  WHERE deleteid='0' AND order_base='1' $where ORDER BY id DESC LIMIT $offset, $pagesize");
                $result = $query->result();
            } else {
                $queryss = $this->db->query("SELECT count(id) as totalcount  FROM $tablename  WHERE deleteid='0' AND order_base='1' AND finance_status='" . $order_base . "'   ORDER BY id DESC");
                $resultcount = $queryss->result();
                foreach ($resultcount as $cc) {
                    $count = $cc->totalcount;
                }
                $query = $this->db->query("SELECT * FROM $tablename  WHERE deleteid='0' AND order_base='1' AND finance_status='" . $order_base . "' $where ORDER BY id DESC LIMIT $offset, $pagesize");
                $result = $query->result();
            }
        }
        foreach ($result as $value) {
    ...
    public function fetch_data_table_trasport_base() {
        ...
        if($assign==11)
        {
             $where.= " AND loading_status='0'";
             $sqls= " AND loading_status='0'";
        ...
        if ($this->session->userdata['logged_in']['access'] == '12') {
            $queryss = $this->db->query("SELECT count(id) as totalcount  FROM $tablename  WHERE deleteid='0' AND order_base='1' AND finance_status='" . $order_base . "' AND assign_status='" . $assign . "' AND user_id='" . $this->userid . "' $sqls  ORDER BY id DESC");
            $resultcount = $queryss->result();
            foreach ($resultcount as $cc) {
                $count = $cc->totalcount;
            }
            $query = $this->db->query("SELECT * FROM $tablename  WHERE deleteid='0' AND order_base='1' AND finance_status='" . $order_base . "' AND assign_status='" . $assign . "' AND user_id='" . $this->userid . "' $where ORDER BY id DESC LIMIT $offset, $pagesize");
            $result = $query->result();
        } elseif ($this->session->userdata['logged_in']['access'] == '11') {
            $sales_team_id = array();
            $query = $this->db->query("SELECT id FROM `admin_users`  WHERE define_saleshd_id='" . $this->userid . "'");
            ...
            $sales_team_id = "'" . implode("','", $sales_team_id) . "'";
            $userslog = ' AND  user_id IN (' . $sales_team_id . ')';
            $queryss = $this->db->query("SELECT count(id) as totalcount  FROM $tablename  WHERE deleteid='0' AND order_base='1' AND finance_status='" . $order_base . "' AND assign_status='" . $assign . "' $userslog $sqls  ORDER BY id DESC");
            $resultcount = $queryss->result();
            foreach ($resultcount as $cc) {
                $count = $cc->totalcount;
            }
            $query = $this->db->query("SELECT * FROM $tablename  WHERE deleteid='0' AND order_base='1' AND finance_status='" . $order_base . "' AND assign_status='" . $assign . "' $where $userslog ORDER BY id DESC LIMIT $offset, $pagesize");
            $result = $query->result();
        } elseif ($this->session->userdata['logged_in']['access'] == '16') {
            $sales_team_id = array();
            $query = $this->db->query("SELECT id FROM `sales_group`  WHERE sales_group_head='" . $this->userid . "'");
            ...
            $query = $this->db->query("SELECT * FROM $tablename  WHERE deleteid='0' AND order_base='1' AND finance_status='" . $order_base . "' AND assign_status='" . $assign . "' $userslog $where ORDER BY id DESC LIMIT $offset, $pagesize");
            $result = $query->result();
        } else {
            $queryss = $this->db->query("SELECT count(id) as totalcount  FROM $tablename  WHERE deleteid='0' AND order_base='1' AND finance_status='" . $order_base . "' AND assign_status='" . $assign . "' $sqls  ORDER BY id DESC");
            $resultcount = $queryss->result();
            foreach ($resultcount as $cc) {
                $count = $cc->totalcount;
            }
            ...
            $query = $this->db->query("SELECT * FROM $tablename  WHERE deleteid='0' AND order_base='1' AND finance_status='" . $order_base . "' AND assign_status='" . $assign . "' $where ORDER BY id DESC LIMIT $offset, $pagesize");
            $result = $query->result();
        }
        foreach ($result as $value) {
            $tablename_sub = "order_product_list_process";
            ...
            $array[] = array('no' => $i, 'id' => $value->id, 'base_id' => base64_encode($value->id), 'order_no' => $value->order_no, 'name' => $company_name, 'email' => $email, 'order_byd' => $order_byd, 'totalamount' => round($discountfulltotal), 'commission' => round($commission), 'delivery_charge' => $value->delivery_charge, 'assign_status' => $value->assign_status, 'phone' => $phone, 'reason' => $value->reason, 'address' => $address, 'order_by' => $order_by, 'order_base' => $value->finance_status, 'create_date' => date('d-m-Y', strtotime($value->create_date)), 'create_time' => $value->create_time);
            $i++;
        }
        $myData = array('PortalActivity' => $array, 'totalCount' => $count);
        echo json_encode($myData);
    ...
    public function fetch_data_table_transpot_by_server() {
        ...
        $array = array();
        if ($route_id != '0') {
            $queryss = $this->db->query("SELECT count(id) as totalcount  FROM $tablename  WHERE deleteid='0'  AND delivery_status='1' AND route_id='" . $route_id . "' AND order_base='1'  AND finance_status='" . $order_base . "' $where ORDER BY id DESC");
            $resultcount = $queryss->result();
            foreach ($resultcount as $cc) {
                $count = $cc->totalcount;
            }
            $query = $this->db->query("SELECT * FROM $tablename  WHERE deleteid='0' AND delivery_status='1' AND order_base='1' AND route_id='" . $route_id . "' AND finance_status='" . $order_base . "'  $where ORDER BY id DESC LIMIT $offset, $pagesize");
        } else {
            $queryss = $this->db->query("SELECT count(id) as totalcount  FROM $tablename  WHERE deleteid='0'  AND delivery_status='1' AND order_base='1'  AND finance_status='" . $order_base . "' $where ORDER BY id DESC");
            $resultcount = $queryss->result();
            foreach ($resultcount as $cc) {
                $count = $cc->totalcount;
            }
            $query = $this->db->query("SELECT * FROM $tablename  WHERE deleteid='0' AND order_base='1'  AND delivery_status='1'  AND finance_status='" . $order_base . "' $where ORDER BY id DESC LIMIT $offset, $pagesize");
        }
        $result = $query->result();
        $i = 1;
        foreach ($result as $value) {
    ...
    public function fetch_data_table_transpot_by_server_own() {
        ...
        $array = array();
        if ($route_id != '0') {
            $queryss = $this->db->query("SELECT count(id) as totalcount  FROM $tablename  WHERE deleteid='0'  AND delivery_status='2' AND route_id='" . $route_id . "' AND order_base='1'  AND finance_status='" . $order_base . "' $where ORDER BY id DESC");
            $resultcount = $queryss->result();
            foreach ($resultcount as $cc) {
                $count = $cc->totalcount;
            }
            $query = $this->db->query("SELECT * FROM $tablename  WHERE deleteid='0' AND delivery_status='2' AND order_base='1' AND route_id='" . $route_id . "' AND finance_status='" . $order_base . "'  $where ORDER BY id DESC LIMIT $offset, $pagesize");
        } else {
            $queryss = $this->db->query("SELECT count(id) as totalcount  FROM $tablename  WHERE deleteid='0'  AND delivery_status='2' AND order_base='1'  AND finance_status='" . $order_base . "' $where ORDER BY id DESC");
            $resultcount = $queryss->result();
            foreach ($resultcount as $cc) {
                $count = $cc->totalcount;
            }
            $query = $this->db->query("SELECT * FROM $tablename  WHERE deleteid='0' AND order_base='1'  AND delivery_status='2'  AND finance_status='" . $order_base . "' $where ORDER BY id DESC LIMIT $offset, $pagesize");
        }
        $result = $query->result();
        $i = 1;
        foreach ($result as $value) {
    ...
    public function fetch_data_table_transpot_vehicle() {
        ...
        $i = 1;
        foreach ($result as $value) {
            $array[] = array('no' => $i, 'vehicle_id' => $value->vehicle_id, 'route_name' => $value->route_name, 'vehicle_number' => $value->vehicle_number, 'vehicle_name' => $value->vehicle_name, 'driver_phone' => $value->driver_phone, 'driver_name' => $value->driver_name);
            $i++;
        }
        echo json_encode($array);
    }
    ...
    public function fetch_data_table_transpot_vehicle_delivered_order_list_by_id() {
        ...
        foreach ($result as $value) {
            ...
            $totalamount = 0;
            $resultpp = $this->db->query("SELECT SUM(rate*qty) as totalamount,SUM(rate*modify_qty) as mtotalamount FROM order_product_list_process  WHERE order_id='" . $value->order_id . "' AND paricel_mode='" . $paricel_mode . "' AND deleteid=0");
            $results = $resultpp->result();
            foreach ($results as $valuep) {
                if ($paricel_mode == 1) {
                    $totalamount = round($valuep->mtotalamount);
                } else {
                    $totalamount = round($valuep->totalamount);
                }
            }
            $minisroundoff = $value->roundoff;
            $roundoffstatus = $value->roundoffstatus;
    ...
    public function cash_mode() {
        ...
        $i = 1;
        foreach ($result as $value) {
            $denomination_total = 10 * $value->c10rs + 20 * $value->c20rs + 50 * $value->c50rs + 100 * $value->c100rs + 200 * $value->c200rs + 500 * $value->c500rs + 2000 * $value->c2000rs;
            if ($collecttion_id == 1) {
                $denomination_total = $denomination_total - $return_excess;
            }
            $array = array('no' => $i, 'order_id' => $value->order_id, 'c10rs' => $value->c10rs, 'c20rs' => $value->c20rs, 'c50rs' => $value->c50rs, 'c100rs' => $value->c100rs, 'c200rs' => $value->c200rs, 'c500rs' => $value->c500rs, 'c2000rs' => $value->c2000rs, 'c10rs_s' => 10 * $value->c10rs, 'c20rs_s' => 20 * $value->c20rs, 'c50rs_s' => 50 * $value->c50rs, 'c100rs_s' => 100 * $value->c100rs, 'c200rs_s' => 200 * $value->c200rs, 'c500rs_s' => 500 * $value->c500rs, 'c2000rs_s' => 2000 * $value->c2000rs, 'denomination_total' => $denomination_total);
            $i++;
        }
        echo json_encode($array);
    }
    ...
    public function fetch_data_table_transpot_assign_data() {
        ...
        if ($dateval == '0') {
            $date = date('Y-m-d');
            if ($route_id != 0) {
                $result = $this->Main_model->where_names_five_order_by_new($tablename, 'finance_status', $order_base, 'order_base', '1', 'deleteid', '0', 'route_id', $route_id, 'assign_date', $date, 'assign_date_time', 'DESC');
            } else {
                $result = $this->Main_model->where_names_four_order_by_new($tablename, 'finance_status', $order_base, 'order_base', '1', 'deleteid', '0', 'assign_date', $date, 'assign_date_time', 'DESC');
            }
        } else {
            if ($route_id != 0) {
                $result = $this->Main_model->where_names_five_order_by_new($tablename, 'finance_status', $order_base, 'order_base', '1', 'deleteid', '0', 'route_id', $route_id, 'assign_date', $dateval, 'assign_date_time', 'DESC');
            } else {
                $result = $this->Main_model->where_names_four_order_by_new($tablename, 'finance_status', $order_base, 'order_base', '1', 'deleteid', '0', 'assign_date', $dateval, 'assign_date_time', 'DESC');
            }
        }
        foreach ($result as $value) {
            $tablename_sub = "order_product_list_process";
    ...
    public function fetch_data_table_transpot_assign_data_driver_list_limit() {
        ...
        if ($this->session->userdata['logged_in']['access'] == '1' || $this->session->userdata['logged_in']['access'] == '6') {
            ...
            $queryss = $this->db->query("SELECT count(id) as totalcount  FROM $tablename  WHERE deleteid='0' AND order_base='1' AND finance_status='" . $order_base . "' AND loading_status='0'  AND assign_status='".$assigen_status."'   ORDER BY id DESC");
            $resultcount = $queryss->result();
            foreach ($resultcount as $cc) {
                $count = $cc->totalcount;
            }
            $query = $this->db->query("SELECT * FROM $tablename  WHERE deleteid='0' AND order_base='1' AND finance_status='" . $order_base . "'  AND loading_status='0' AND assign_status='".$assigen_status."'  ORDER BY id DESC LIMIT $offset, $pagesize");
            $result = $query->result();
        ...
        else
        {
               ...
               $queryss = $this->db->query("SELECT count(id) as totalcount  FROM $tablename  WHERE deleteid='0' AND order_base='1' AND finance_status='" . $order_base . "' AND loading_status='0' AND driver_id='".$define_driver_id."'   ORDER BY id DESC");
            $resultcount = $queryss->result();
            foreach ($resultcount as $cc) {
                $count = $cc->totalcount;
            }
            $query = $this->db->query("SELECT * FROM $tablename  WHERE deleteid='0' AND order_base='1' AND finance_status='" . $order_base . "'  AND loading_status='0' AND driver_id='".$define_driver_id."'  ORDER BY id DESC LIMIT $offset, $pagesize");
            $result = $query->result();
    ...
    public function fetch_single_data_totaldel() {
        ...
        foreach ($result as $value) {
            if ($value->paricel_mode == 1) {
                $amounttotals+= round($value->rate * $value->modify_qty + $value->commission, 3);
                $amounttotal_with_out_commissions+= round($value->rate * $value->modify_qty, 3);
                $fullqtys+= $value->modify_qty;
            }
            $amounttotal+= round($value->rate * $value->qty + $value->commission, 3);
            $amounttotal_with_out_commission+= round($value->rate * $value->qty, 3);
            $fullqty+= $value->qty;
            $amounttotalgst+= round($value->rate * $value->qty * $value->gst / 100, 3);
            $Meter_to_Sqr_feet+= $value->Meter_to_Sqr_feet;
            $Sqr_feet_to_Meter+= $value->Sqr_feet_to_Meter;
            $commission+= $value->commission;
            $nos+= $value->nos;
    ...
    public function fetchproduct_fullmm_val() {
        ...
        $convert = $form_data->convert;
        $array = array();
        $result = $this->Main_model->where_names('product_list', 'product_name', $product_id);
    ...
    public function payment_collected() {
        ...
        if ($collectamount != "" && $drivercharge != "") {
            ...
            $results = $resultpp->result();
            foreach ($results as $valuep) {
                $totalamount = $valuep->totalamount + $delivery_charge;
            }
            if ($roundoffstatus == 1) {
            ...
            if ($payment_mode == 'Cash') {
                ...
                $data_address['account_head_id'] = 36;
            } else {
            ...
            $res = $this->Main_model->where_names_limit_base('customer_ledger', 'customer_id', $customer_id, 1);
            $balancetotal = 0;
            $debitsamount = 0;
            $creditsamount = 0;
            foreach ($res as $val) {
            ...
            $res = $this->Main_model->where_names_limit_base('driver_ledger', 'customer_id', $driver_id, 1);
            $balancetotaldd = 0;
            ...
            $data_driver['notes'] = 'Derivery Payment ' . $trip_id;
            ...
            if ($driver_payment_mode == 'Cash') {
                $data_driver['account_head_id'] = 36;
                if ($checkid == 1) {
            ...
            $data_driver['process_by'] = 'Driver Order Reconciliation';
            ...
            foreach ($result_order as $orders) {
                ...
                foreach ($result_order_product as $orders_product) {
                    ...
                    if ($oldtablename_sub == 'order_product_list_process') {
                        $this->db->query("UPDATE product_list SET stock=stock-'" . $orders_product->qty . "' WHERE id='" . $orders_product->product_id . "'");
    ...
    public function order_quotation_move_finish() {
        ...
        if ($deleteid == 0) {
            ...
            foreach ($result_order as $orders) {
                ...
                $find['reason'] = 'Process To Transpot';
                $find['others'] = $orders->others;
                $find['print'] = $orders->print;
                $find['packaging'] = $orders->packaging;
                $find['competitorname'] = $orders->competitorname;
            ...
            $data_address['customer_id'] = $customer_id;
            $data_address['user_id'] = $this->userid;
            $data_address['notes'] = 'Order Debit ' . $find['order_no'];
            $data_address['payment_mode'] = $payment_mode;
            $data_address['order_no'] = $find['order_no'];
            $data_address['difference'] = 0;
            $data_address['reference_no'] = '';
            $data_address['process_by'] = 'Order Amount Debited';
            ...
            $data_address['payment_date'] = $date;
            $data_address['payment_time'] = $time;
            $this->Main_model->insert_commen($data_address, 'customer_ledger');
        } else {
            $point['get_id'] = $form_data->order_no;
            $point['order_base'] = 0;
    ...
    public function order_quotation_move_finish_by_deilvered() {
        ...
        $data_address['customer_id'] = $customer_id;
        $data_address['user_id'] = $this->userid;
        $data_address['notes'] = 'Order Debit ' . $order_no;
        $data_address['payment_mode'] = $payment_mode;
        $data_address['order_no'] = $order_no;
        $data_address['difference'] = 0;
        $data_address['reference_no'] = '';
        ...
        $data_address['payment_date'] = $date;
        $data_address['payment_time'] = $time;
        $result_order_product = $this->Main_model->where_names_two_order_by('customer_ledger', 'customer_id', $customer_id, 'order_no', $order_no, 'id', 'ASC');
        if (count($result_order_product) == 0) {
            $this->Main_model->insert_commen($data_address, 'customer_ledger');
        }
        // Once production completed change value 0
        $datassfirst['reason'] = "Process To Transpot";
        $this->Main_model->update_commen($datassfirst, $tablenamemain);
        $datassfirst1['get_id'] = $form_data->order_id;
        // $datassfirst1['order_base']=1;
        $datassfirst1['reason'] = "Process To Transpot";
    ...
    public function fetchCustomerororderhistroyorderlist_driver() {
        ...
        foreach ($resultorder as $value) {
            ...
            $array[] = array('no' => $i, 'id' => $value->id, 'imagestatus' => $imagestatus, 'order_id' => $value->order_id, 'order_no' => $value->order_no, 'product_name_tab' => $product_name, 'categories' => $categories, 'description' => $description, 'product_id' => $value->product_id, 'tile_material_name' => $value->tile_material_name, 'tile_material_id' => $value->tile_material_id, 'categories_id' => $value->categories_id, 'profile_tab' => $value->profile, 'crimp_tab' => $value->crimp, 'dim_two' => $value->dim_two, 'dim_one' => $value->dim_one, 'dim_three' => $value->dim_three, 'image_length' => $value->image_length, 'gst' => $value->gst, 'gst_check' => $value->gst_check, 'extra_crimp' => $value->extra_crimp, 'back_crimp' => $value->back_crimp, 'nos_tab' => $value->nos, 'unit_tab' => $value->unit, 'return_status' => $value->return_status, 'fact_tab' => $value->fact, 'uom' => $value->uom, 'billing_options' => $value->billing_options, 'commission_tab' => $value->commission, 'categories_id_get' => $categories_id, 'Meter_to_Sqr_feet' => round($value->Meter_to_Sqr_feet, 3), 'Sqr_feet_to_Meter' => round($value->Sqr_feet_to_Meter, 3), 'rate_tab' => $value->rate, 'qty_tab' => $value->qty, 'amount_tab' => round($amount, 4));
            $i++;
        }
        echo json_encode($array);
    }
    ...
    public function orderassign() {
        ...
        for ($i = 0;$i < count($orderid_data);$i++) {
            ...
            if ($vehicle_id_data != '') {
                ...
                $datass['reason'] = 'Ready for delivery';
                $datass['otp'] = '1234';
    ...
    public function fetch_data_table_date_waise() {
        ...
        $array = array();
        if ($this->session->userdata['logged_in']['access'] == '12') {
            $result = $this->Main_model->where_names_three_order_by_betweendate($tablename, 'order_base', $order_base, 'user_id', $this->userid, 'deleteid', '0', 'id', 'DESC', $formdate, $todate);
            if (count($result) == 0) {
                $result = $this->Main_model->where_names_three_order_by_betweendate($tablename, 'order_base', $order_base, 'entry_user_id', $this->userid, 'deleteid', '0', 'id', 'DESC', $formdate, $todate);
            }
        } elseif ($this->session->userdata['logged_in']['access'] == '11') {
            $sales_team_id = array();
            $query = $this->db->query("SELECT id FROM `admin_users`  WHERE define_saleshd_id='" . $this->userid . "'");
            ...
            $result = $this->Main_model->where_names_three_thried_where_in_order_by_betweendate($tablename, 'order_base', $order_base, 'deleteid', '0', 'user_id', $sales_team_id, 'id', 'DESC', $formdate, $todate);
        } elseif ($this->session->userdata['logged_in']['access'] == '16') {
            $sales_team_id = array();
            $query = $this->db->query("SELECT id FROM `sales_group`  WHERE sales_group_head='" . $this->userid . "'");
            $resultsales_team = $query->result();
            ...
            $result = $this->Main_model->where_names_three_thried_where_in_order_by_betweendate($tablename, 'order_base', $order_base, 'deleteid', '0', 'sales_group', $sales_team_id, 'id', 'DESC', $formdate, $todate);
        } else {
            $result = $this->Main_model->where_names_two_order_by_betweendate($tablename, 'order_base', $order_base, 'deleteid', '0', 'id', 'DESC', $formdate, $todate);
        }
        if ($tablename == 'orders') {
            $tablename_sub = "order_product_list";
        }
