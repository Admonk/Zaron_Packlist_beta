                                             ...
                                             <div class="card-header align-items-start d-flex p-3">
                                                <div class="flex-shrink-0">
                                                   <ul class="nav justify-content-end nav-tabs-custom rounded card-header-tabs" role="tablist" ng-init="getcount('orders_process')">
                                                      ...
                                                      <li class="nav-item">
                                                         <a class="nav-link" data-bs-toggle="tab" href="#messages2" role="tab" ng-click="pageTab2('orders_process',111)">
                                                         <span class="d-block d-sm-none"><i class="far fa-envelope"></i></span>
  ...
  $scope.error = false;
   ...
   function getData2() {
      ...
      var order_base = $('#order_base').val();
         ...
         $http.get('<?php echo base_url() ?>index.php/order/fetch_data_table_po?page=' + $scope.currentPage + '&size=' + $scope.pageSize + '&search=' + $scope.searchText + '&tablename=' + $scope.tablename+'&order_base='+order_base)
        .success(function(data) {
            $scope.namesData = [];
            $scope.totalItems = data.totalCount;
            $scope.startItem = ($scope.currentPage - 1) * $scope.pageSize + 1;
...
$scope.onPre = function(currentPage,pageSize){
  ...
  function getDataPage(currentPage,pageSize) {
        ...
        if(status==111)
        {
            $http.get('<?php echo base_url() ?>index.php/order/fetch_data_table_po?page_next=' + currentPage + '&size=' + pageSize + '&search=' + $scope.searchText + '&tablename=' + $scope.tablename+'&order_base='+order_base)
            .success(function(data) {
                $scope.namesData = [];
                $scope.totalItems = data.totalCount;
