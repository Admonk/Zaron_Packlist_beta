                                                ...
                                                <div class="flex-shrink-0">
                                                   <ul class="nav justify-content-end nav-tabs-custom rounded card-header-tabs" role="tablist" ng-init="getcount('orders_process')">
                                                        ...
                                                        <li class="nav-item" >
                                                         ...
                                                         <span class="d-none d-sm-block">Production Print <?php echo $text; ?></span> 
                                                         </a>
                                                         </li>
                                                         
                                                         
                                                          <li class="nav-item" >
                                                         <a class="nav-link"  href="<?php echo base_url() ?>index.php/order_second/production_print_log_details" role="tab">
                                                         
                                                             <span class="d-sm-block"> Print Log</span> 
                                                         </a>
                                                         </li>
...
<button onclick="window.print()" class="btn btn-primary" ng-click="Print()" style="margin: 25px 0px;">Print</button>
                           ...
                           <a href="<?php echo base_url(); ?>index.php/order/production_print_canceled" class="btn btn-default" style="margin: 25px 0px;">Cancel Orders</a>
                           ...
                           {
                               ?>
                               <a href="<?php echo base_url(); ?>index.php/order/production_print?status_ssd=1" class="btn btn-default" style="margin: 25px 0px;">SDP Orders</a>
                           ...
                           ?>
                             ...
                             <div class="col-sm-4">
                                                    <div id="example_filter" class="dataTables_filter">
                                                        <label for="prod_categoriesSelect">Product Category</label>
                                                        <select class="select2 form-control  multiple-select" required multiple="multiple" name="product_categories[]" id="product_categories" ng-modal="product_categories" >
                       ...
                       <div class="card" style="margin : 10px; padding: 5px;">
                                                <h6 style="padding-top: 10px;"><b style="color :red" >Selected Categories : </b><span class="valuecategory"></span></h6>         
                                            </div>
                                                                  ...
                                                                  <thead>
                                                                      <tr>
                                                                                                      <th style="width:2%;">#</th>
                                                                                                        ...
                                                                                                        <th style="width:35%;"><b>Attachment</b></th>
                                                ...
                                                if ($this->session->userdata['logged_in']['access'] == '1' || $this->session->userdata['logged_in']['access'] == '15') {
                                                  ...
                                                  ?> 
<b >{{name.order_no}} | {{name.company_name}} | {{name.create_date}}</b>
                                             ...
                                             <input type="hidden" class="valorder" value="{{name.order_id}}">
                                                                      ...
                                                                      <tr>
                                                                       ...
                                                                       <tr ng-repeat="names in name.subarray" class="{{names.color_style}}">
                                                                                                        ...
                                                                                                        <p ng-if="names.subvalues" class="res">{{names.subvalues}}</p>
                                                                                              ...
                                                                                              <span ng-if="names.uom_image">UOM : {{names.uom_image}}</span>
                                                                                                            ...
                                                                                                            <!--<a href="{{names.reference_image}}" style="color: blue;" target="_blank">Attachment</a>-->
                                                                                                             ...
                                                                                                             <span ng-if="names.remarks" class="res"> <br> Remarks :{{names.remarks}}</span>       
     ...
     <script>
        $(document).ready(function() {
            $('.multiple-select').select2();
                        ...
                        var productionPrintValues = "<?php echo $production_print; ?>";
                        var selectedValuesArray = productionPrintValues.split(',').map(function(item) {
                            return item.trim(); // Trim spaces if present
                        });

                        // Pre-select options using Select2 method
                        $('.multiple-select').val(selectedValuesArray).trigger('change');

                        var selectedCategories = $('#product_categories option:selected').map(function() {
                            return $(this).text();
                        }).get();

                        console.log('Selected Category Names:', selectedCategories);

                        var buttonsHTML = ''; // Initialize an empty string to store button HTML

                        selectedCategories.forEach(function(category) {
                            buttonsHTML += '<button class="btn btn-outline-primary btn-sm" style="margin-right : 3px; opacity: initial !important;" disabled>' + category + '</button>';
                        });

                        $('.valuecategory').html(buttonsHTML); // Update div content with selected categories as buttons
...
app.controller('crudController', function($scope, $http){
  ...
  $scope.to_date = new Date();
    ...
    var selectedCategories = "<?php echo $production_print; ?>";
    getData(selectedCategories);



   function getData(selectedCategories) 
   {
     ...
     $http.get('<?php echo base_url() ?>index.php/order/fetch_data_production_order_by_print?order_id=0&cancelid=0&status_ssd=<?php echo $status; ?>&tablenamemain=<?php echo 'orders_process'; ?>&&tablename_sub=<?php echo 'order_product_list_process'; ?>&convert=1&production_id=1&status=0&from_date='+from_date+'&to_date='+to_date+'&order_no1='+order_no1+'&order_no2='+order_no2+ '&categories=' + selectedCategories)
   ...
   $scope.searchTextChanged = function() {
    var selectedCategories = $('#product_categories').val();
        getData(selectedCategories);
    }
