...
<body data-layout="horizontal" class="full-vh-overflow-hide" data-topbar="dark" ng-app="crudApp" ng-controller="crudController">
            ...
            <div class="page-content">
               <div class="container-fluid">
                                                ...
                                                <div class="flex-shrink-0">
                                                   <ul class="nav justify-content-end nav-tabs-custom rounded card-header-tabs" role="tablist" ng-init="getcount('orders_process')">
                                                      ...
                                                      <!-- <li class="nav-item ">
                                                       ...
                                                       <li class="nav-item ">
                                                         <a class="nav-link <?php echo $active1; ?>"  href="<?php echo base_url(); ?>index.php/order/finance_orders_list?status=5"  >
                                                         <span class="d-block d-sm-none"><i class="far fa-envelope"></i></span>
                                                         <span class="d-none d-sm-block">Reconciliation Completed Orders &nbsp;&nbsp;<span class="badge rounded-pill bg-success float-end"> {{totalItems}} </span></span>   
                                                         </a>
                                                      </li> 
                                              ...
                                              <div class="col-12">
                                                  <div class="card shadow-none">
                                                      <div class="card-body">
                                                            ...
                                                            <input type="hidden" id="order_base" value="<?php echo $status ?>">
                        ...
                        <div id="example_filter" class="dataTables_filter">
                            <label>Search:</label><input type="search" class="form-control input-sm" placeholder="Order No,Name,phone,Sales Person,Driver,Vehicle_No" aria-controls="example" ng-model="searchText" ng-change="searchTextChanged()">
                        </div>
                    </div>
                       </div>
                                                                      ...
                                                                      <tr  ng-repeat="name in namesData">
                                                                          ...
                                                                          <!--<td>{{name.commission}}</td>-->
                                                                           ...
                                                                           <td>
                                                                               ...
                                                                               <span ng-if="name.driver_recived_payment>0">{{name.driver_recived_payment | number}}</span>
                                                                           </td>
                                                                           <!-- <td>{{name.delivery_charge}}</td> -->
                                                                           <td>{{name.delivery_status}}</td>
                                                   ...
                                                   <a href="javascript:void(0);" ng-click="getorderid(name.id,name.order_no,name.selforder,name.order_byd,name.vehicle_number,name.randam_id)" ng-if="name.finance_status==5"><i class="mdi mdi-eye font-size-16 text-success me-1"></i> Payment View</a>
                                                                                      
                                                                                     
                                                                                     <button  ng-if="name.finance_status==6" type="button" ng-click="onview(name.totalamount,name.customer_id,name.order_no,name.id,name.payment_id_base)"  class="btn btn-soft-success  waves-effect waves-light" >Receive Payment</button>
    ...
    <div class="modal fade" id="firstmodalcommison1" aria-hidden="true" aria-labelledby="exampleModalToggleLabel" tabindex="-1">
                                          ...
                                          <div class="card-body" ng-repeat="namesorderid in namesDataorderid">
                                                ...
                                                <h5 ng-if="namesorderid.commision_value>0">Commission (+) : <b >Rs. {{namesorderid.commision_value | number}}</b></h5>
...
app.controller('crudController', function($scope, $http){
  ...
  $scope.error = false;
   ...
   function getData() {
     ...
     $http.get('<?php echo base_url() ?>index.php/order/fetch_data_table_finance?page=' + $scope.currentPage + '&size=' + $scope.pageSize + '&search=' + $scope.searchText + '&tablename=' + $scope.tablename+'&order_base='+order_base+'&from_date='+from_date+'&to_date='+to_date)
...
$scope.onPre = function(currentPage,pageSize){
  ...
  function getDataPage(currentPage,pageSize) {
      ...
      var to_date = $('#to_date').val();
        ...
        $http.get('<?php echo base_url() ?>index.php/order/fetch_data_table_finance?page_next=' + currentPage + '&size=' + pageSize + '&search=' + $scope.searchText + '&tablename=' + $scope.tablename+'&order_base='+order_base+'&from_date='+from_date+'&to_date='+to_date)
        .success(function(data) {
            $scope.namesData = [];
            $scope.totalItems = data.totalCount;
...
$scope.getcount = function (tabelename) {
 ...
 $http({
  ...
  $scope.getorderid = function(id,name,selforder,drivename,vichalnumber,randam_id) {
  ...
  $scope.cashmode = function(tabelename,status,id,randam_id){
    ...
    $http.get('<?php echo base_url() ?>index.php/order/cash_mode?tablename='+tabelename+'&order_base='+status+'&order_id='+id+'&randam_id='+randam_id).success(function(data){
