...
<div class="modal fade bs-example-modal-center" id="re_date" tabindex="-1" role="dialog" aria-hidden="true">
                                                ...
                                                <div class="flex-shrink-0">
                                                   <ul class="nav justify-content-end nav-tabs-custom rounded card-header-tabs" role="tablist" >
                                                       ...
                                                       <li class="nav-item ">
                                                         ...
                                                         <span class="d-sm-block">Dispatch Un-Approvel list </span>   
                                                         </a>
                                                      </li> 
                                                                  ...
                                                                  <tbody>
                                                                      ...
                                                                      <tr  ng-repeat="name in namesData" >
                                                                        ...
                                                                        <td><a href="<?php echo base_url(); ?>index.php/order/ordercreate_product_process?order_id={{name.base_id}}">
                                                                          ...
                                                                          <td><a href="<?php echo base_url(); ?>index.php/order/ordercreate_product_process?order_id={{name.base_id}}">{{name.order_byd}}</a></td>
                                                                          <td><a href="<?php echo base_url(); ?>index.php/order/ordercreate_product_process?order_id={{name.base_id}}">{{name.vehicle_number}}</a></td>
                        ...
                        <button type="button"  ng-click="coniformDate('1')" class="btn btn-success waves-effect waves-light" >Confirm</button>
                        <button type="button"  ng-click="coniformDate('-1')" class="btn btn-primary waves-effect waves-light" >Decline</button>
...
app.controller('crudController', function($scope, $http){
  ...
  $scope.to_date = new Date();
   ...
   function getData() {
     ...
     $http.get('<?php echo base_url() ?>index.php/order/fetch_data_table_dispatch_unapprovel_list?page=' + $scope.currentPage + '&size=' + $scope.pageSize + '&search=' + $scope.searchText + '&tablename=' + $scope.tablename+'&order_base='+order_base+'&from_date='+from_date+'&to_date='+to_date)
...
$scope.onPre = function(currentPage,pageSize){
  ...
  function getDataPage(currentPage,pageSize) {
      ...
      var to_date = $('#to_date').val();
        ...
        $http.get('<?php echo base_url() ?>index.php/order/fetch_data_table_delivery?page_next=' + currentPage + '&size=' + pageSize + '&search=' + $scope.searchText + '&tablename=' + $scope.tablename+'&order_base='+order_base+'&from_date='+from_date+'&to_date='+to_date)
        .success(function(data) {
            $scope.namesData = [];
            $scope.totalItems = data.totalCount;
...
$scope.cancelData = function(id){
  ...
  $scope.coniformDate = function(status){
        ...
        if($('.customerlistcheck').is(':checked'))
        {
             ...
             if(status==1)
             {
                   var st="Dispatch Approved";
             }
             else
             {
                   var st="Dispatch Decline";
             }
             ...
             $('input[name="customerlistcheck"]:checked').each(function(){
                ...
                $http({
                  method:"POST",
                  url:"<?php echo base_url() ?>index.php/order/dispatch_approved",
                  data:{'order_id':checkinsert,'status':status}
                }).success(function(data){
