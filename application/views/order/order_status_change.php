  ...
  <div class="col-xl-12 mb-4" ng-repeat="name in namesData">
    <div class="card">
      <div class="card-body">
          ...
          <div class="d-flex align-items-center">
            ...
            <div class="col-md-3" >
             ...
             <div class="col-md-3">
              ...
              <p class="fw-bold mb-1">Delivery charges : {{name.delivery_charge}}</p>    
            ...
            <div class="col-md-3">
             ...
             <div class="col-md-3">
                     ...
                     <span ng-if="name.order_base==0">
                      ...
                      <span ng-if="name.km_reading_end>0">, End Reading KM : {{name.km_reading_end}}</span>
                      </p>
        ...
        <div class="col-xl-12 mb-4" ng-if="name.viewstatus==1">
        
    <label class="radio-inline">
      <input type="radio" name="optradio_{{name.randam_id}}" value="1" ng-checked="name.statuscheck==1" > Order Created
    </label>

    <label class="radio-inline">
      <input type="radio" name="optradio_{{name.randam_id}}" value="7" ng-checked="name.statuscheck==7" > Order Date Confirmed
    </label>

    <label class="radio-inline">
      <input type="radio" name="optradio_{{name.randam_id}}" value="2" ng-checked="name.statuscheck==2" > Order Assigned
    </label>
    <label class="radio-inline">
      <input type="radio" name="optradio_{{name.randam_id}}" value="3" ng-checked="name.statuscheck==3"> Dispatch  Loaded
    </label>
    <label class="radio-inline">
      <input type="radio" name="optradio_{{name.randam_id}}" value="4" ng-checked="name.statuscheck==4"> Driver  Loaded
    </label>
    <label class="radio-inline">
      <input type="radio" name="optradio_{{name.randam_id}}" value="5" ng-checked="name.statuscheck==5"> Driver Trip Started
    </label>

    <label class="radio-inline">
      <input type="radio" name="optradio_{{name.randam_id}}" value="6" ng-checked="name.statuscheck==6"> Driver Trip Completed  
    </label>


    <label class="radio-inline">
      <input type="radio" name="optradio_{{name.randam_id}}" value="9" ng-checked="name.statuscheck==9"> Reconciliation
    </label>


    <label class="radio-inline">
      <input type="radio" name="optradio_{{name.randam_id}}" value="8" ng-checked="name.statuscheck==8"> In Production Return
    </label>
    ...
    <?php
     ...
     ?>
  <label class="radio-inline">
      <button class="btn btn-outline-danger" ng-click="saveOrder(name.id,name.order_no,name.randam_id)">SAVE</button>
    </label>
...
<?php
 ...
 $access=array('1','10','6','21','15');
     if(in_array($this->session->userdata['logged_in']['access'], $access))
     {
        ...
        <span class="btn btn-outline-danger font-size-11" ng-if="name.assign_status==11"><a href="#" ng-click="removeAssign(name.id,names.randam_id)">Un-Assign</a></span>
       ...
       <span class="btn btn-outline-primary font-size-11" ng-if="name.assign_status==11 || name.assign_status==1 || name.loading_status==1"><a href="<?php echo base_url(); ?>index.php/order/pickup_orders_list_view?id={{name.id}}&driver_pickip=0&DC_id={{name.randam_id}}" target="_blank">Loading / Pickup </a></span>
...
$scope.saveOrder = function(id,order_no,randam_id)
{
     
     var checked=$("input[name='optradio_"+randam_id+"']:checked").val();
   ...
   if(confirm("Are you sure you want to status change?"))
  {
        ...
        data:{'id':id,'checked':checked,'order_no':order_no,'randam_id':randam_id, 'action':'order_full_status_change','tablename':'orders_process'}
        }).success(function(data){
       
         
         alert('success');
         $('.btn-outline-danger').html('SAVE');
