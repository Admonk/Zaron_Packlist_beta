                  ...
                  <div class="col-md-3">
                     <div class="route-sidebar w-100" style="min-width: unset;">  
                           ...
                           <ul class="list-unstyled chat-list" ng-init="fetchVehicle(4)" >
                                ...
                                <li  ng-repeat="namesvehicle in namesDatavehicle | filter:queryv" ng-class="{activess : activeMenuf === namesvehicle.vehicle_id}" class="{{namesvehicle.bgcolor}}">
                  ...
                  <input type="hidden" id="v_id">
                    ...
                    <div class="col-md-5" >
                     <div class="active-order-sidebar h-100">
                        ...
                        <div class="row" ng-init="setOrders()">
                           <div class="col-12">
                              <div class="active-trip-tracking p-3" ng-init="fetchRouteorder('orders_process',2,0)">
                                    ...
                                    <div class="card" ng-repeat="names in namesData.orders | filter:query" ng-if="names.trip_id==namestrip">
                                          <div class="card-body" ng-click="getorderid_1(names.order_id,names.order_no)" ng-class="{activess : activeMenu === names.order_id}">
                                              
                                              
                                           
                                              
                                             <h5 class="card-title font-size-13">

<input type="number" value="{{names.sort_id}}" class="setsort_id" id="sort_id_{{names.randam_id}}"  style="width: 50px;text-align: center;" ng-keyup="sortidchange(names.order_id,names.randam_id)" >



 <a href="<?php echo base_url(); ?>index.php/order/pickup_orders_list_view?id={{names.order_id}}&driver_pickip=0&DC_id={{names.randam_id}}" >
                                              - Order ID : {{names.order_no}}  <small class="text-truncate mb-0 midtext"> {{names.delivery_mode}} Delivery</small>

                                               <p class="text-muted float-end">
                                              <span class="badge rounded-pill badge-soft-secondary font-size-11"><a href="#"
                                              ng-click="removeAssign(names.order_id,names.randam_id)"><i class="mdi mdi-key-remove font-size-16 text-danger me-1"></i> Un-Assign</a></span> |


                                              <span class="badge rounded-pill badge-soft-secondary font-size-11" ><a href="#"  ng-click="removeAssignCallback(names.id,names.randam_id)"><i class="mdi mdi-call-split font-size-16 text-danger me-1"></i>  Call Back</a></span>
                                             </p>
                                             ...
                                             <p class="text-muted float-end">
                                              ...
                                              <span ng-if="names.text_delivery_status=='Own Scope'">
                                                <b> Delivery Date  : {{names.delivery_date_time}}</b> 
                                              <br>
                                             </span>

                                             <b> Assign Date  : {{names.trip_end_date}} {{names.trip_end_time}} </b></p>
                                             ...
                                             <p class="text-muted ">Vehicle No {{names.vehicle_name}} | {{names.vehicle_number}}  </p>
                                              ...
                                              <p class="text-muted ">Scope : {{names.text_delivery_status}}  </p>

                                              <p class="text-muted ">Last Status : {{names.reason}}  </p>
                                            
                                             <p  ng-if="names.delivery_status==2"> 
                                            <span ng-if="names.seq_status==0" style="color: red;">

                                           Status : Driver Sequencing is Pending

                                            </span> 
                                              
                                            <span ng-if="names.seq_status==1" style="color: green;">

                                           Status : Driver Sequencing is Completed 
                                             ...
                                             <p class="text-muted float-end" ng-if="names.delivery_status==2"> 
                                             
                                             
                                             
                                              <span class="badge rounded-pill badge-soft-success font-size-12" ng-if="names.seq_status==1">

                                                <a href="<?php echo base_url(); ?>index.php/order/pickup_orders_list_view?id={{names.order_id}}&driver_pickip=0&DC_id={{names.randam_id}}" style="display: block;padding: 8px 25px;" ><i class="mdi mdi-briefcase-upload font-size-16 text-danger me-1"></i>  Load </a>

                                              </span>


                                               <span class="badge rounded-pill badge-soft-secondary font-size-12" ng-if="names.seq_status==0">

                                                         
                                               <a href="javascript:void(0);" style="display: block;padding: 8px 25px;">
                                               <i class="mdi mdi-briefcase-upload font-size-16 text-danger me-1"></i>  Load </a>
                                                ...
                                                <p class="text-muted float-end" ng-if="names.delivery_status==1"> 
...
app.controller('crudController', function($scope, $http){
 ...
 $scope.checkCustomerpay = function() {
  ...
  $scope.setOrders = function(){
    ...
    $http.get('<?php echo base_url() ?>index.php/order_check/order_delivery_order_status_update').success(function(data){});
...
//{
  ...
  var sort_id=$('#sort_id_'+randam_id).val();
  ...
  if(reason!='')
  {
    ...
    if(confirm("Are you sure you want to unassign?"))
    {
      $http({
        ...
        data:{'id':id,'reason':reason,'randam_id':randam_id, 'action':'removeassign','tablenamemain':'orders_process'}
      }).success(function(data){
          $('#firstmodalcommison').modal('toggle');
        $scope.success = false;
        $scope.error = false;
