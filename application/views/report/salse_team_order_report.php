...
<body data-layout="horizontal" class="full-vh-overflow-hide" data-topbar="dark" ng-app="crudApp" ng-controller="crudController">
 ...
 $array = array(11,12);
 if(!in_array($this->session->userdata['logged_in']['access'], $array)) 
 {
?>
                                           <div class="col-2">
        ...
        app.controller('crudController', function($scope, $http) {
            ...
            $scope.fetchDatagetlegderGroup = function(cateid, productid, fromdate, fromto, sales_group, status) {
                ...
                $http.get('<?php echo base_url() ?>' + url + '?limit=10&formdate=' + fromdate + '&todate=' + fromto +
                    ...
                    '&sales_group=' + sales_group + '&active_status=' + status).success(function(data) {
                    // $('.spinnerCls').hide();
                    $scope.query = {}
                    $scope.queryBy = '$';
                    ...
                    setTimeout(function() {
                        data[0].map((el) => {
                            el.salesperson && el.salesperson.map((el1) => {
                                ...
                                for ($i = 0; $i <= 11; $i++) {
                                ?>
                                    $(`td[data-sl-id="${el1.sales_team_id}"][data-month-index="<?= $i ?>"]`).html(formatVal(el1.data.find(el => el.index == '<?= $i ?>').target));
                                <?php
                                }
                                ?>
            ...
            $('#sales_group_target, #from-month').on('change optionselected optiondeselected', function() {
                ...
                for (let i = 0; i < headings.length; i++) {
                    ...
                    // console.log("dateInfo", dateState.toISOString())
                    var formattedDate = dateState.toISOString().slice(0, 10);
                    // let index = $(headings[i]).attr('data-month-index');
