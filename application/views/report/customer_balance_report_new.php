...
<div class="alert alert-primary" role="alert" style='    margin:50px 50px;'>
File updates are in progress in this page. Kindly ignore the value difference or loading issue. The page will resume back shortly.
</div>
                      ...
                      <form>
                          <div class="row">
                            ...
                            <div class="col">
                             ...
                             <button type="button"  class="btn d_none btn-success waves-effect waves-light <?php echo $d_none; ?>" id="exportdata" >Export</button>
...
<div id="resp-table">
    <div id="resp-table-body" style="position: relative;">
        <div class="resp-table-row" style="position: sticky;top: 0;background: #dfdfdf;" class="table table-bordered"  ng-init="fetchDatagetlegderGroup(0,0)"> 
                          ...
                          <div  class="table-body-cell <?php echo $d_none; ?>" style="width: 150px;">Customer_Name</div>
                          <div  class="table-body-cell <?php echo $d_none; ?>">Customer_phone</div>
                          <div  class="table-body-cell <?php echo $d_none; ?>" style="width: 150px;">Area</div>
  ...
  $('#payment_mode_payoff').on('change',function(){
   ...
   $(".Uncheck").on('click',function(){
     ...
     var val=$(this).val();
      ...
      $.ajax({
      url: '<?php echo base_url() ?>index.php/Report_new_customer_balence/table_customize',
...
$('#exportdata').on('click', function() {
      ...
      url = '<?php echo base_url() ?>index.php/Report_new_customer_balence/<?=$url_name?>limit=10&customer_id='+customer_id+'&cate_id='+cateid+'&sales_group='+sales_group+'&productid='+productid+'&formdate='+fromdate+'&todate='+fromto+'&order_status='+order_status+'&payment_mode=1';
      location = url;
...
$('#exportdata_static').on('click', function() {
      ...
      var sales_group= $('#sales_group').val();
      var payment_mode= $('#payment_mode').val();
      // url = '<?php echo base_url() ?>index.php/Report_new_customer_balence/<?=$url_name?>formdate='+fromdate+'&todate='+fromto+'&data='+
       // JSON.stringify($scope.namesDataledgergroup)+'&totals='+JSON.stringify($scope.totalsStatic) ;
      // location = url;
 $http({
         method:"POST",
         url:'<?php echo base_url() ?>index.php/Report_new_customer_balence/<?=$url_name?>',
         data:{
