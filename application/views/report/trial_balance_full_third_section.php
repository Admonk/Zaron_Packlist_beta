...
<body data-layout="horizontal" class="full-vh-overflow-hide" data-topbar="dark" ng-app="crudApp" ng-controller="crudController">
 ...
 <div class="main-content">
  ...
  <!-- Main Data -->
  <tr ng-if="namesvv.set > 0" ng-click="viewSub(namesvv.id)">
    <td>
      <b>
        <a href="#" style="color: #ef7b3f; font-weight: 800;">{{ namesvv.account_type_name }}</a>
      </b>
      <i class="fa fa-angle-down" style="float: right;"></i>
    </td>
    <td>
      <b ng-if="namesvv.debit != 0"><a href="#">{{ namesvv.debit | indianCurrency }}</a></b>
    </td>
    <td>
      <b ng-if="namesvv.credit != 0"><a href="#">{{ namesvv.credit | indianCurrency }}</a></b>
    </td>
    <!-- <td><pre>{{ namesvv.sub | json }}</pre></td> -->
  </tr>

  <!-- Sub Array -->
  <span ng-repeat="names in namesvv.sub track by $index" ng-if="names.set > 0">
    <tr >
    <td><a href="{{ names.url }}">{{ names.name }}</a></td>
    <td ng-if="names.debit > 0">{{ names.debit | indianCurrency }}</td>
    <td ng-if="names.credit > 0">{{ names.credit | indianCurrency }}</td>

  </tr>

<tr ng-repeat="namesval in names.third_array ">
    <td>{{ namesval.namess }}</td>
    <td>{{ namesval.totaldebit | indianCurrency }}</td>
    <td>{{ namesval.totalcridit | indianCurrency }}</td>
  </tr>
  </span>
...
$scope.fetchDatagetlegderGroup = function(fromdate,fromto,search){
  ...
  var fromdate= $('#from-date').val();
    ...
    $http.get('<?php echo base_url() ?>index.php/report_new/trial_balance_report_full_beta_test_third?limit=10&formdate='+fromdate+'&fromto='+fromto+'&accountstype=<?php echo $accountstype; ?>').success(function(data){
