...
$scope.onview = function(id,billno,pendingamount,resno){
 ...
 $scope.exportToExcel = function () {
  ...
  $scope.fetchDatagetlegderGroupcloseingbalance = function(id,fromdate,fromto,payment_status,party_type){
    ...
    $http.get('<?php echo base_url() ?>index.php/report/day_book_all_view_balance?limit=10&customer_id='+id+'&formdate='+fromdate+'&todate='+fromto+'&party_type='+party_type+'&payment_status='+payment_status).success(function(data){
       ...
       // console.log($scope.totalcridit)
        ...
        $scope.opening_balance =  parseInt(data.opening_balance);
 ...
 $scope.loading = true;
    ...
    $http.get('<?php echo base_url() ?>index.php/report/day_book_all_view_beta?limit=10&customer_id='+id+'&formdate='+fromdate+'&todate='+fromto+'&party_type='+party_type+'&payment_status='+payment_status).success(function(data){
     ...
     var total_debits =0;
      ...
      $scope.totalcridit =parseInt(total_credits + total_credits_bank);
      $scope.totaldebit = parseInt(total_debits + total_debits_bank);


     if($scope.opening_status == 1){
      $scope.totalcridit = parseInt(total_credits + total_credits_bank + $scope.opening_balance);
     }else{
      $scope.totaldebit = parseInt(total_debits + total_debits_bank + $scope.opening_balance);
     }


        $scope.closing_balance = Math.abs($scope.totalcridit - $scope.totaldebit);
