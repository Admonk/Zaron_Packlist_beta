...
<div id="layout-wrapper" ng-init="getData()">
      <?= $top_nav; ?>
                           ...
                           <div class="col-md-6">
                              ...
                              <table class="table">
                                 ...
                                 <tbody>
                                    ...
                                    <td>
                                    <input type="hidden" id="id_l" ng-model = "row.id" />
                                       ...
                                       <td ng-show="select_type == '1'"> <select class="form-control" id="category_id_{{row.id}}" name="category_id" ng-model="category_id">
                                          ...
                                          <td ng-show="select_type == '2'" > <select class="form-control" id="sub_category_id_{{row.id}}" name="sub_category_id"  ng-model="sub_category_id">
                                       ...
                                       <td> <input type="text" id="autocomplete_{{row.id}}" ng-keyup="completeProduct(row.id)" class="form-control rounded border autocomplete" placeholder="Product"  ng-model="row.product_id" ></td>
                                       ...
                                       <td><input type="text" class="form-control-dual values-left"  id="value_{{row.id}}" ng-model="row.value" ng-init="getTotalDebitFun()" ng-change="getTotalDebitFun()" placeholder="?" /></td>
                              ...
                              <table class="table">
                                 ...
                                 <tbody>
                                    <tr ng-repeat="row in rightTableRows">
                                       <td>
                                       <input type="hidden" id="id_r" ng-model = "row.id" />
                                       ...
                                       <td ng-show="select_type_r == '1'" > <select class="form-control" id="category_id_r{{row.id}}" name="category_id"  ng-model="category_id_r">
                                          ...
                                          <td ng-show="select_type_r == '2'" > <select class="form-control" id="sub_category_id_r{{row.id}}" name="sub_category_id_r"  ng-model="sub_category_id_r">
                                       ...
                                       <td> <input type="text" id="autocomplete_r{{row.id}}" ng-keyup="completeProductR(row.id)" name="profile" class="form-control rounded border autocomplete" placeholder="Product" ng-model="row.product_id"></td>
                                       ...
                                       <td><input type="text" class="form-control-dual" id="value_r{{row.id}}" placeholder="?" ng-model="row.value" ng-init="getTotalCreditFun()" ng-change="getTotalCreditFun()" /></td>
                                       <td>
   ...
   <script>
      ...
      app.controller('JournalController', ['$scope', '$http', '$q', function($scope, $http, $q) {
         ...
         $scope.getData = function () {
            ...
            $http({
               method: "POST",
               url: "<?php echo base_url() ?>index.php/stock_journal/fetch_data",
               data: {
                'product_id' : product_id
               }
            }).success(function(data) {
               $scope.journalNo = data.journal_no;

                console.log(data);
                console.log(data.type_l);

                $scope.select_type = parseFloat(data.type_l);
                $scope.select_type_r = parseFloat(data.type_r);

                $scope.category_id = parseFloat(data.category_id_l);
                $scope.category_id_r = parseFloat(data.category_id_r);

                $scope.sub_category_id = parseFloat(data.sub_category_l);
                $scope.sub_category_id_r = parseFloat(data.sub_category_r);

                var prod_l = data.product_id_l+"-"+data.product_name_l;
                var prod_r = data.product_id_r+"-"+data.product_name_r;


            $scope.leftTableRows = [{
                selectedid: '',
                product_id: prod_l,
                // productName: data.product_name_l,
                quantity: parseFloat(data.quantity_l),
                uom: '40',
                rate: data.rate_l,
                coilNumber: data.coil_no_l,
                value: data.debit_l,
                id: data.id_l
            }];
            ...
            $scope.rightTableRows = [{
                selectedid: '',
                product_id: prod_r,
                // productName: data.product_name_r,
                quantity: parseFloat(data.quantity_r),
                uom: '40',
                rate: data.rate_r,
                coilNumber: data.coil_no_r,
                value: data.credit_r,
                id: data.id_r
            }];
            ...
            if($scope.getTotalDebit == 0 && $scope.getTotalCredit == 0){
               ...
               angular.forEach($scope.leftTableRows, function(row) {
                  if (row.value !== null) {
                     ...
                     let subcatId = type == "2" ? $('#sub_category_id_'+row.id).val() : "0";
                     ...
                     $http({
                        method: "POST",
                        url: "<?php echo base_url() ?>index.php/stock_journal/stock_update",
                        data: {
                           'action': 'debit',
                           'catId' : catId,
                           'subcatId' : subcatId,
               ...
               angular.forEach($scope.rightTableRows, function(row) {
                  if (row.value !== null) {
                     ...
                     let subcatId = type == "2" ? $('#sub_category_id_r'+row.id).val() : "0";
                     let qty = row.quantity;
                     let value = row.value;
                     let coilNumber = row.coilNumber;
                     let id = row.id;
