                     ...
                     <div class="row align-items-start">
                        ...
                        <div class="col-md-3">
                           ...
                           <div class="input-group date">
                              <input id="vch_date" type="date" min="<?php echo date('2024-08-01'); ?>" name="selectedDateModels" ng-model="selectedDateModels"  class="form-control"   value="{{selectedDateModels}}" ng-model-options="{ getterSetter: true }">
                           ...
                           <tr class="trpoint" ng-repeat="row in tableRows track by $index">
                              <td>
                                 <div class="input-group">
                                    <select class="form-control" ng-disabled="isSelectDisabled(row)" name="row.selectedOption"  ng-model="row.selectedOption" ng-change="onTypeChange(row)" required>
                              ...
                              <td style="width: 150px;">
                                 <div class="input-group">
                                    <input type="number" ng-model="row.debit" ng-change="onChangeDr(row)" ng-enter="addRow()" class="form-control no-spinner" placeholder="₹" ng-disabled="checkDisable(row)">
                                    

                                 </div>
                                 <span style="margin-left: 20px; color:blue;" ng-if="row.current_balance_debit" ><b> Bal : {{row.current_balance_debit | number}}</b></span>
                              </td>
                              <td style="width: 150px;">
                                 <div class="input-group">
                                    <input type="number" ng-model="row.credit" ng-change="onChangeCr(row)" ng-enter="addRow()" class="form-control no-spinner" placeholder="₹" ng-disabled="checkDisableCr(row)">
                                    <span class="btn btn-outline-danger" ng-if="$index > 0" ng-click="removeRow($index)">-</span>
                                 </div>
                                 <span style="margin-left: 20px; color:blue;" ng-if="row.current_balance_credit" ><b>Bal : {{row.current_balance_credit | number}}</b></span>
                              </td>
                           </tr>
                        </tbody>
                     </table>
...
app.filter('number', function () {        
   ...
   app.controller('JournalController', ['$scope', '$http' ,'$q', function($scope, $http, $q) {
      ...
      $scope.selectedDateModels = new Date();
      $scope.dayOfWeek = '';
      $scope.tableRows = [{
         selectedid: '',
         debit: null,
      ...
      $scope.isSelectDisabled = function(row) {
         return row.disabled;  
      };
      ...
      $scope.onChangeDr = function(div){
        const availableBl = Math.abs(div.opening_balance);

        if(div.debit >= 0){
        div.current_balance_debit = availableBl - div.debit;
        }

      //   else if(div.credit >= 0){
         
      //    div.current_balance_credit = availableBl + div.credit;
      // }
      }

      $scope.onChangeCr = function(div){
         const availableBl = Math.abs(div.opening_balance);
 ...
 $scope.checkDisableCr =function(row){
   if($scope.VoucherTypeStatus != 'journal'){
         return row.debit !== null && row.debit !== '' || row.selectedOption === 'dr'

      }else{
         return row.selectedOption === 'dr';
      }
 }
   $scope.checkDisable = function(row){
      if($scope.VoucherTypeStatus != 'journal'){
         return row.credit !== null && row.credit !== '' || row.selectedOption === 'cr'

      }else{
         return row.selectedOption === 'cr';
      }
   }
    ...
    $scope.onTypeChange = function(div){
      ...
      if($scope.VoucherTypeStatus == 'journal'){
         if(div.selectedOption == 'cr'){
            div.credit = div.debit;
            div.debit = 0;
         }else{
            div.debit = div.credit;
            div.credit = 0;
         }
      }
      ...
      // }else{
         // $scope.error_msg='Please select Dr first.';
      // }
    ...
    $scope.Getbankaccount = function (row) {
      ...
      $scope.addRow = function() {
         ...
         if($scope.VoucherTypeStatus == 'journal'){
            ...
            // $scope.selectedType = '';
            cr_val =  parseFloat(totalDebit - totalCredit);
            $scope.addRowAllowed=true;
      ...
      $scope.selectedDateModel = function(newDate) {
         ...
         if (new_date) {
            // $scope.updateDayOfWeek(new_date);
      ...
      $scope.saveData = function() {
         ...
         if($scope.selectedVoucherType == 'contra'){
            ...
            endpoint = 'index.php/journal/save_data?cr_type=' + cr_type + '&cr_party=' + cr_party + '&dr_type=' + dr_type + '&dr_party=' + dr_party;
         }
