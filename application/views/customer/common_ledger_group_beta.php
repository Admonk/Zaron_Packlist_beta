                      ...
                      <form>
                          <div class="row">
                            ...
                            <div class="col" style="display:none;">
                             ...
                             <button type="button" class="btn btn-success waves-effect waves-light" ng-click="exportToExcel()" id="exportdata1" >Export</button>
                            </div>
                   ...
                   <table id="datatable"  class="table table-striped table-bordered" style="position: relative;" width="100%" ng-init="fetchDatagetlegderGroup(0)" >
                      <thead>
                        <tr style="position: sticky;top: 0;background: #dfdfdf;">
                          ...
                          <th style="width:100px;"><h5 class="font-size-14 mb-0">Name</th>
                          <th style="width:100px;"><h5 class="font-size-14 mb-0">Opening Balance</h5></th>
                        ...
                        <tbody  ng-repeat="namesvv in namesDataledgergroup" >
                        ...
                        <tr  class="trpoint" >
                           ...
                           <td>{{namesvv.no}}</td>
                           <!--  <td>{{namesvv.vl}}</td> -->
                           <td><a href="{{namesvv.urls}}" target="_blank">{{namesvv.name}}</a></td>
                           <td><a href="{{namesvv.urls}}" target="_blank"><span ng-if="namesvv.opening_balance!=0">{{namesvv.opening_balance | number}} {{namesvv.payment_status}}</span></a></td>
                           <td><a href="{{namesvv.urls}}" target="_blank"><span ng-if="namesvv.debits!=0">{{namesvv.debits | number}}</span></a></td>
                           <td><a href="{{namesvv.urls}}" target="_blank"><span ng-if="namesvv.credits!=0">{{namesvv.credits | number}}</span></a></td>
                           <td>
                             <span ng-if="namesvv.balance>0">
                             <a href="{{namesvv.urls}}" target="_blank"><span  ng-if="namesvv.getstatus==1" style="color:red">{{namesvv.balance | number}}</span>
                       ...
                       <!-- <tr  class="trpoint" ng-repeat="name in namesvv.subarray" style="font-size: 10px;font-weight:600;display:none;">
                          
                           <td></td>
                            <td>{{namesvv.vl}}</td>
                           <td></td>
                           <td><a href="{{name.url}}" target="_blank"><span ng-if="name.opening_balance!=0">{{name.opening_balance | number}} {{name.payment_status}}</span></a></td>
                           <td><a href="{{name.url}}" target="_blank"><span ng-if="name.debits!=0">{{name.debits | number}}</span></a></td>
                           <td><a href="{{name.url}}" target="_blank"><span ng-if="name.credits!=0">{{name.credits | number}}</span></a></td>
                           <td>
                            <span ng-if="name.balance>0">
                             <a href="{{name.url}}" target="_blank"><span  ng-if="name.getstatus==1" style="color:red">{{name.balance | number}}</a></span>
                             <span  ng-if="name.getstatus==0" style="color:green">{{name.balance | number}}</span></a>
                             </span>
                           </td>
...
$scope.fetchSingleData = function(id){
  ...
  $scope.exportToExcel = function () {
                ...
                $("#datatable").table2excel({
                    filename: "Commen_leger_group_"+date+'_'+dateto+".xls"
                });
 };
...
$scope.fetchDatagetlegderGroup = function(id,fromdate,fromto,payment_status){
     ...
     var trail_balance = '<?php echo $party_type; ?>';
...
$scope.loading = true;
    ...
    $http.get('<?php echo base_url() ?>index.php/customer/fetch_data_get_ledger_view_group_commen_beta?limit=10&customer_id='+id+'&formdate='+fromdate+'&todate='+fromto+'&payment_status='+payment_status).success(function(data){
     ...
     $scope.loading = false;
      ...
      $scope.namesDataledgergroup = data;
       ...
       $('#choices-single-default').attr('readonly',false);
        ...
        var totalcriditval = 0;
        for(var i = 0; i < data.length; i++){
            var amount = parseInt(data[i].credits);
            totalcriditval += amount;
        }
      
      
      
        $scope.totalcridit = totalcriditval;
        ...
        var totaldebitval = 0;
        for(var i = 0; i < data.length; i++){
            var balance = parseInt(data[i].debits);
            totaldebitval += balance;
        }
        ...
        $scope.totaldebit = totaldebitval;
        ...
        var payouttotal = 0;
        for(var i = 0; i < data.length; i++){
            var Payout = parseInt(data[i].balance_tot);
            payouttotal += Payout;
        }
