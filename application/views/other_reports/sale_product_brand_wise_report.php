...
.tooltip {
    z-index: 1000 !important; /* or a higher value */
                            ...
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                ...
                                <div class="page-title-right">
                                    <ol class="breadcrumb m-0">
                                        ...
                                        <li class="breadcrumb-item active">Sale Product Brand wise Report</li>
                                    </ol>
                                </div>
                                ...
                                <div class="row" style="margin: 13px 0px;">
                                    <div class="col-6 mb-2">
                                        <div class="page-title-box d-sm-flex align-items-center justify-content-between p-0">
                                            <h4 class="mb-sm-0 font-size-18">Sale Product Brand wise Report <?=$testMode ? '- (Order Detailed View)' : ''?></h4>
                                        </div>
                                    </div>
                                    <div class="col-lg-12">
                                        <form>
                                            <div class="row">
                                                ...
                                                <div class="col">
                                                    <select class="filter-multi-select" name="states[]" multiple="multiple" id="sales_group_target">
                                                        ...
                                                        ?>
                                                            <option value="<?php echo $val->categories; ?>"><?php echo $val->categories; ?></option>
                                                        <?php
                                                        }
                                                        ?>
                                                    </select>
                                                ...
                                                <div class="col text-end">
                                                   <!--  <i type="button" class="fa fa-info" data-bs-toggle="tooltip" data-bs-html="true" title="
                                                    <b>Sale Value</b> - {{value | number}}<br>
                                                    <b>Area Report :</b>{{area_value | number}}<br/>
                                                    <b>Roundoff :</b> {{roundoff | number}}<br/>
                                                    <b>Auto Roundoff :</b> {{autoRound | number}}<br/>
                                                    <b>Discount :</b> {{discount | number}}<br/>
                                                    <b>Overall ~ </b> {{ overall | number }}<br/>
                                                        ...
                                                        <div class="table-responsive table-container" style="height: 700px;" ng-init="fetchSalesProductReportData(0,0)">
                                                            <table class="main" style="width:100%">
                                                                ...
                                                                <tr ng-repeat="(index, row) in salesData" >
                                                                    <td>{{index + 1}}</td>
                                                                    <td class="cursor-pointer" ng-click="viewBrand(row.categories)">{{row.categories ? row.categories : 'No Brand'}}</td>
      ...
      return function(input) {
        ...
        app.controller('crudController', function($scope, $http) {
            ...
            $scope.viewBrand = function (brand){
                var fromdate = $('#from-date').val();
                var fromto = $('#to-date').val();
                let sales_brand = brand;

                  <?php
                    $url = ($testMode == true) ? 'sale_product_brand_cate_report?test=true&' : 'sale_product_brand_cate_report?' ;
                ?>

                let url = '<?php echo base_url(); ?>index.php/other_reports/<?php echo $url; ?>limit=10&formdate=' + fromdate + '&todate=' + fromto + '&sales_brand=' + sales_brand
            ...
            $scope.fetchSalesProductReportDataExport = function() {
                ...
                if(sales_brand[0]['states[]'] && sales_brand[0]['states[]'].includes('All')){
                  ...
                  <?php
                    $url_name = $testMode ? 'fetch_data_sales_product_brand_wise_report_export?test=true&':'fetch_data_sales_product_brand_wise_report_export?';
                ?>
                let url = '<?php echo base_url() ?>index.php/other_reports/<?=$url_name?>limit=10&formdate=' + fromdate + '&todate=' + fromto + '&brand=' + cateid
                window.location = url;
            }
            ...
            $scope.fetchSalesProductReportData = function() {
                ...
                <?php
                    $url_name = $testMode ? 'fetch_data_sales_product_brand_wise_report?test=true&':'fetch_data_sales_product_brand_wise_report?';
                ?>

                $http.get("<?php echo base_url() ?>index.php/other_reports/<?=$url_name?>limit=10&formdate=" + fromdate + '&todate=' + fromto + '&brand=' + cateid ).success(function(data) {
                    ...
                    Object.keys(data['totals']).map((el) => {
                        $scope[el] = (data['totals'][el] * Math.pow(10, 2) / Math.pow(10, 2)).toFixed(2);
                    })
