...
<body data-layout="horizontal" class="full-vh-overflow-hide" data-topbar="dark" ng-app="crudApp" ng-controller="crudController">
                            ...
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                ...
                                <div class="page-title-right">
                                    <ol class="breadcrumb m-0">
                                        ...
                                        <li class="breadcrumb-item active">Sale Product Area Category Report</li>
                                    </ol>
                                </div>
                                ...
                                <div class="row" style="margin: 13px 0px;">
                                    <div class="col-6 mb-2">
                                        <div class="page-title-box d-sm-flex align-items-center justify-content-between p-0">
                                            <h4 class="mb-sm-0 font-size-18">Sale Product Area Category Report ({{selectedCustomer}}) </h4>
                                        </div>
                                    </div>
                                    <div class="col-lg-12">
                                        <form>
                                                        ...
                                                        <div class="table-responsive table-container" style="height: 700px;" ng-init="fetchSalesProductReportData(0,0)">
                                                            <table class="main" style="width:100%">
                                                                ...
                                                                <tr ng-repeat="(index, row) in salesData" >
                                                                    <td>{{index + 1}}</td>
                                                                    <td class="cursor-pointer" ng-click="viewArea(row.categories)">{{row.categories}}</td>
    ...
    return function (input) {
        ...
        app.controller('crudController', function($scope, $http) {
            ...
            $scope.viewArea = function (cate){
                var fromdate = $('#from-date').val();
                var fromto = $('#to-date').val();
                let category = encodeURIComponent(cate);
                let sales_area = encodeURIComponent('<?= $_GET['sales_area'] ?>');
                let sales_customer = encodeURIComponent('<?= $_GET['sales_customer'] ?>');
                let url = '<?php echo base_url() ?>index.php/other_reports/sale_product_area_detail_report?limit=10&formdate=' + fromdate + '&todate=' + fromto + '&sales_area=' + sales_area + '&category=' + category + '&sales_customer=' + sales_customer
            ...
            $scope.fetchSalesProductCateReportDataExport = function() {
               ...
               $scope.loading = true;
                ...
                var fromdate = $('#from-date').val();
                var fromto = $('#to-date').val();
                var sales_area = '<?= $_GET['sales_area'] ?>';
                var sales_customer = '<?= $_GET['sales_customer'] ?>';
               
                let url ='<?php echo base_url() ?>index.php/other_reports/fetch_data_sales_product_area_cate_wise_report_export?limit=10&formdate=' + fromdate + '&todate=' + fromto + '&sales_customer=' + sales_customer;
                window.location = url;
            }
            ...
            $scope.fetchSalesProductReportData = function() {
                ...
                $http.get('<?php echo base_url() ?>index.php/other_reports/fetch_data_sales_product_area_cate_wise_report?limit=10&formdate=' + fromdate + '&todate=' + fromto + '&sales_customer=' + sales_customer ).success(function(data) {
                    $scope.query = {}
                    $scope.queryBy = '$';
                    ...
                    $scope.selectedCustomer = data['data'][0]['customer_name'];
