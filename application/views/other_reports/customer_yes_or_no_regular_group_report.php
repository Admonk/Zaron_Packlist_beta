                            ...
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                ...
                                <div class="page-title-right">
                                    <ol class="breadcrumb m-0">
                                        ...
                                        <li class="breadcrumb-item active">Regular Consolidated Report</li>
                                    </ol>
                                </div>
                                ...
                                <div class="row" style="margin: 13px 0px;">
                                    <div class="col-6 mb-2">
                                        <div class="page-title-box d-sm-flex align-items-center justify-content-between p-0">
                                            <h4 class="mb-sm-0 font-size-18">Regular Consolidated Report</h4>
                                        </div>
                                    </div>
                                    <div class="col-lg-12">
                                        <form>
                                                            ...
                                                            <table class="main" style="width:100%">
                                                                ...
                                                                <thead style="position: sticky;top: 50px;z-index: 3;">
                                                                    <tr>
                                                                        ...
                                                                        <th class="second-cell" style="">Salesman</th>
    ...
    <script>
        ...
        app.controller('crudController', function($scope, $http) {
            ...
            $scope.fetchSalesProductReportDataExport = function() {
                ...
                let url = '<?php echo base_url() ?>index.php/other_reports/fetch_data_customer_yes_or_no_group_export?limit=10&formdate=' + fromdate + '&todate=' + fromto + '&sales_person=' + cateid;
                window.location = url;
            }
            ...
            $scope.fetchSalesProductReportData = function() {
                ...
                $http.get('<?php echo base_url() ?>index.php/other_reports/fetch_data_customer_yes_or_no_group?limit=10&formdate=' + fromdate + '&todate=' + fromto + '&sales_person=' + cateid).success(function(data) {
                    $('table tbody').empty();
                    $scope.monthHeads = [];
                    ...
                    let monthHeads = ['Billed Regular Customers', 'Total Regular Customers', 'No. of Bills', 'Percentage (Billed Regular Customers / Overall Regular Customers)'];
                    let totalKeys = ['regular_customers', 'regular_customers_create', 'bill_count', 'percentage'];
                    ...
                    Object.keys(data['data']) && Object.keys(data['data']).map((el, i) => {
                        htmlData += `<tr class="tbody-tr-head" style=" background-color:#FFD966;box-shadow: inset 1px 0px 0px 2px #e0e0e0;">
                          
                                               <td class="column2 first-cell" style="position: sticky;left: 0px;">${inn}</td>

                                               <td  class="column2 second-cell" ><b>${el  ? el : 'Others'}</b></td>`;

// console.log("data['data']", data['data'][el]);
                        for (var i = 0; i < $scope.monthNameHeads.length; i++) {
                                 htmlData += `<td class="text-center"><b>${data['data'][el]['regular_customers_'+($scope.monthNameHeads[i].no)]}</b></td>
                                               <td class="text-center"><b>${data['data'][el]['regular_customers_create_'+($scope.monthNameHeads[i].no)]}</b></td>
                                               <td class="text-center"><b>${data['data'][el]['total_orders_'+($scope.monthNameHeads[i].no)]}</b></td>
                                               <td class="text-center"><b>${formatNumber(data['data'][el]['percentage_'+($scope.monthNameHeads[i].no)])} %</b></td>`;
                        }

                        htmlData += `</tr>`;

                        inn++;
                        let filteredData = Object.keys(data['data'][el])
                            .filter(key => !isNaN(parseInt(key)))
                            .reduce((obj, key) => {
                                obj[key] = data['data'][el][key];
                                return obj;
                            }, {});
                        let indexIn = 1;
                        ...
                        for (let key in filteredData) {
                            ...
                            if (filteredData[key].overall_regular_customers > 0) {
                                htmlData += `<tr class="tbody-tr">
                                                                        <td  style="position: sticky;left: 0px;" class="text-center first-cell column1">${ Number(indexIn)}</td>
                                                                        <td  class="cursor-pointer text-center column1 second-cell" ng-click="viewSalePerson(row.id)">${filteredData[key].name ? filteredData[key].name : 'No Name'} </td>`;

                                for (var i = 0; i < $scope.monthNameHeads.length; i++) {
                                    htmlData += `<td class="text-center cursor-pointer" ng-click="viewSalePerson(row.id)">${filteredData[key]['regular_customers_'+($scope.monthNameHeads[i].no)] || 0}</td>
                                                                        <td class="text-center cursor-pointer" ng-click="viewSalePerson(row.id)">${filteredData[key]['regular_customers_create_'+($scope.monthNameHeads[i].no)] || 0}</td>
                                                                        <td class="text-center cursor-pointer" ng-click="viewSalePerson(row.id)">${filteredData[key]['bill_count_'+($scope.monthNameHeads[i].no)] || 0}</td>
                                                                        <td class="text-center cursor-pointer" ng-click="viewSalePerson(row.id)">${formatNumber(filteredData[key]['percentage_'+($scope.monthNameHeads[i].no)] || '')} %</td>`;
                                }
                                htmlData += `</tr>`;
                                indexIn++;
                            }
