                            ...
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                ...
                                <div class="page-title-right">
                                    <ol class="breadcrumb m-0">
                                        ...
                                        <li class="breadcrumb-item active">Sale Product Brand Category Report</li>
                                    </ol>
                                </div>
                                ...
                                <div class="row" style="margin: 13px 0px;">
                                    <div class="col-6 mb-2">
                                        <div class="page-title-box d-sm-flex align-items-center justify-content-between p-0">
                                            <h4 class="mb-sm-0 font-size-18">Sale Product Brand Category Report (<?=$_GET['sales_brand'] != 'null' ? $_GET['sales_brand'] : 'No Brand'?>)  <?=$testMode ? '- (Order Detailed View)' : ''?></h4>
                                        </div>
                                    </div>
                                    <div class="col-lg-12">
                                        <form>
                                            <div class="row">
                                                ...
                                                <div class="col">
                                                    ...
                                                    <button ng-click="fetchSalesProductCateReportDataExport()" type="button" class="btn btn-success waves-effect waves-light" id="exportdata">Export</button>
                                                </div>
                                                        ...
                                                        <div class="table-responsive table-container" style="height: 700px;" ng-init="fetchSalesProductReportData(0,0)">
                                                            <table class="main" style="width:100%">
                                                                ...
                                                                <tr ng-repeat="(index, row) in salesData" >
                                                                    <td>{{index + 1}}</td>
                                                                    <td class="cursor-pointer" ng-click="viewBrand(row.categories)">{{row.categories ? row.categories : 'No Category'}}</td>
  ...
  app.filter('number', function() {
      return function(input) {
        ...
        app.controller('crudController', function($scope, $http) {
            ...
            $scope.viewBrand = function (cate){
                var fromdate = $('#from-date').val();
                var fromto = $('#to-date').val();
                let category = encodeURIComponent(cate);
                let sales_brand = encodeURIComponent('<?= $_GET['sales_brand'] ?>');

                 <?php
                    $url = ($testMode == true) ? 'sale_product_brand_detail_report?test=true&' : 'sale_product_brand_detail_report?' ;
                ?>


                let url = '<?php echo base_url() ?>index.php/other_reports/<?php echo $url; ?>limit=10&formdate=' + fromdate + '&todate=' + fromto + '&sales_brand=' + sales_brand + '&category=' + category;
                // console.log("url",url);
                window.open(url,'_blank');
            }
            
            $scope.fetchSalesProductCateReportDataExport = function() {
                
                $scope.loading = true;

                var fromdate = $('#from-date').val();
                var fromto = $('#to-date').val();
                var  sales_brand = '<?= $_GET['sales_brand'] ?>';
                 <?php
                    $url_name = $testMode ? 'fetch_data_sales_product_cate_wise_report_export?test=true&':'fetch_data_sales_product_cate_wise_report_export?';
                ?>
                let url = '<?php echo base_url() ?>index.php/other_reports/<?=$url_name?>limit=10&formdate=' + fromdate + '&todate=' + fromto + '&sales_brand=' + sales_brand
                window.location = url;
            }
            ...
            $scope.fetchSalesProductReportData = function() {
                ...
                <?php
                    $url_name = $testMode ? 'fetch_data_sales_product_cate_wise_report?test=true&':'fetch_data_sales_product_cate_wise_report?';
                ?>

                
                $http.get('<?php echo base_url() ?>index.php/other_reports/<?=$url_name?>llimit=10&formdate=' + fromdate + '&todate=' + fromto + '&sales_brand=' + sales_brand ).success(function(data) {
                    $scope.query = {}
                    $scope.queryBy = '$';
