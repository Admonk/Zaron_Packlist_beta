...
app.controller('crudController', function($scope, $http){
  ...
  $scope.fetchData = function(){
    $http.get('<?php echo base_url() ?>index.php/group/fetch_data_state').success(function(data){
  ...
  $scope.submitForm = function(){
    $http({
      ...
      data:{'name':$scope.name,'id':$scope.hidden_id,'action':$scope.submit_button,'tablename':'state'}
      ...
      if(data.error != '1')
      {
        ...
        $scope.name = "";
        $scope.hidden_id = "";
        $scope.successMessage = "State name successfully "+$scope.submit_button;
        $scope.fetchData();
...
$scope.fetchSingleData = function(id){
    $http({
      ...
      data:{'id':id, 'action':'fetch_single_data','tablename':'state'}
    }).success(function(data){
...
$scope.deleteData = function(id){
    if(confirm("Are you sure you want to remove it?"))
    {
      $http({
        method:"POST",
        url:"<?php echo base_url() ?>index.php/group/insertandupdate",
        data:{'id':id, 'action':'Delete','tablename':'state'}
      }).success(function(data){
        $scope.success = false;
        $scope.error = false;
        $scope.fetchData();
